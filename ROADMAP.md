...
The semantic execution layer is validated locally: build, lint, diff check, and all 64 tests pass. Damage reactions enter through generic event-provider discovery, legacy response requests normalize once at ingress, canonical response/trigger decisions revalidate live providers, and the client uses the generic hidden-hand and Equipment target-card selection contract. Attack, Duel, Group/AOE and Halberd response creators now own their opening event IDs directly. The remaining final-round audit is saved-state compatibility isolation; do not begin Blue Steel Sword before that audit is complete.
