...
Generic provider-owned Judgement is now complete: a provider may return `requires_resolution`, the generic engine reveals a Judgement card, asks that provider to interpret it, and resumes the stored Attack/AOE continuation without an Eight-Trigrams-specific dispatch. Provider discovery also enforces the deliberate interaction invariant: **0–1 implicit ordinary-card provider** and any number of explicit alternatives. Green Dragon Blade and Rock Cleaving Axe now execute as `AttackDodged` triggers: their modules own the follow-up Attack and exact two-card cost validation, while the pending screens are compatibility adapters. Next: migrate Frost Sword through a pre-damage trigger, then replace the global presentation-idle check with a decision-specific `readyAfterEventId` barrier and retire compatibility response actions.
