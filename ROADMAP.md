...
Generic provider-owned Judgement is now complete: a provider may return `requires_resolution`, the generic engine reveals a Judgement card, asks that provider to interpret it, and resumes the stored Attack/AOE continuation without an Eight-Trigrams-specific dispatch. Provider discovery also enforces the deliberate interaction invariant: **0–1 implicit ordinary-card provider** and any number of explicit alternatives. Green Dragon Blade is the first executable `AttackDodged` trigger: its module supplies both the follow-up option and live selected-Attack validation, while the existing pending screen is a compatibility adapter. Next: migrate Rock Cleaving Axe, then Frost Sword, through the trigger contract; afterwards replace the global presentation-idle check with a decision-specific `readyAfterEventId` barrier and retire compatibility response actions.
