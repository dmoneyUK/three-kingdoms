...
The response presentation barrier is complete: every provider, decline action, cost selector and human countdown starts together only after the prior public presentation has settled. Next: persist the semantic requirement as the single canonical pending decision, then route trigger resolution through the same interfaces.
