...
The response presentation barrier is complete: every provider, decline action, cost selector and human countdown starts together only after the prior public presentation has settled. Human timers are armed once and are idempotent across refreshes. The client now renders server-projected provider options generically, including no-cost effects and constrained card-cost effects. Next: persist the semantic requirement as the single canonical pending decision, then route generic Judgement and trigger resolution through the same interfaces.
