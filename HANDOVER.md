...
The room projection now exposes that discovery as a viewer-private `currentAction.requirement`, provider `options`, and `declineAction`. The pure `game/response-decision.ts` module owns this pending-requirement-to-private-options mapping and its generic-provider-to-legacy-resolver adapter, keeping that equipment/capability knowledge out of the HTTP route. The protocol accepts `respond` with `providerId` and optional selected card IDs; the Worker reloads the live room, recomputes the available provider, rejects stale/unavailable providers with the newest room, then delegates to the proven legacy resolver action. The browser now submits this generic path for physical responses, Serpent Spear and Eight Trigrams where it is available. Existing per-provider actions deliberately remain as fallbacks until the full compatibility projection is removed. Next: persist the semantic requirement as the single canonical pending decision, then move passive and triggered effects into their corresponding capability interfaces.
