...
test("client keeps the turn, response, presentation, and selection controls", async () => {
  ...
  assert.match(page, /sequence-mini-cards/);
  ...
  assert.match(page, /ROLE REVEALED/);
  assert.match(page, /ResolutionSequence/);
  assert.match(page, /resolutionEvents/);
  assert.match(page, /CARD RESOLUTION/);
  assert.match(page, /Card effect/);
  assert.match(styles, /\.resolution-sequence-panel/);
  assert.match(styles, /\.resolution-step\.active/);
