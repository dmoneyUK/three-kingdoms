...
try {
  await waitForServer();
  const tests = spawn(process.execPath, ["--test", "--test-concurrency=1", "tests/private-hand.test.mjs", "tests/rendered-html.test.mjs", "tests/game-api.test.mjs"], { cwd: new URL("../", import.meta.url), env: { ...process.env, GAME_TEST_URL: url }, stdio: "inherit" });
