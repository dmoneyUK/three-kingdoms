...
test("pure Dying HP, match outcome, and defeat continuation rules", () => {
  ...
  const continuationPlayers = [{ id: "lord", seat: 0, role: "Lord", alive: 1 }, { id: "rebel", seat: 1, role: "Rebel", alive: 1 }, { id: "dead", seat: 2, role: "Loyalist", alive: 0 }];
  assert.deepEqual(determineDefeatContinuation({ players: continuationPlayers, turnSeat: 2, resumePlayerId: "dead", hasGroupContinuation: true }), { kind: "advance_turn", nextSeat: 0 });
  assert.deepEqual(determineDefeatContinuation({ players: continuationPlayers, turnSeat: 0, resumePlayerId: "lord", hasGroupContinuation: true }), { kind: "resume_group" });
  assert.deepEqual(determineDefeatContinuation({ players: continuationPlayers, turnSeat: 0, resumePlayerId: "lord", hasGroupContinuation: false }), { kind: "resume_effect" });
  assert.deepEqual(determineDefeatContinuation({ players: continuationPlayers.map((player) => player.id === "rebel" ? { ...player, alive: 0 } : player), turnSeat: 0, resumePlayerId: "lord", hasGroupContinuation: false }), { kind: "finish", outcome: "lord_loyalist" });
});
