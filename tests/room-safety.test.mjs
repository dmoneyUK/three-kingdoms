...
test("normalizes the canonical current action without trusting unknown legal actions", () => {
  const room = normalizeRoomData({
    ...
    pending: { kind: "attack" },
    currentAction: { version: 1, kind: "attack", actorId: "p1", deadline: 123, reason: "Dodge or take damage", legalActions: ["respond", "respond", "decline_response", "invent_action"], playPhaseActions: [{ cardId: "red-peach", canPlayAs: "attack" }, { cardId: "red-peach", canPlayAs: "attack" }, { cardId: "bad", canPlayAs: "dodge" }], triggerEvent: "attack_targeted", triggerOptions: [{ effectId: "yin_yang_swords_attack_targeted", label: "Yin-Yang Swords", allowDecline: false, timeoutChoiceId: "draw", selection: { type: "choice", choices: [{ id: "discard", label: "Discard 1 hand card" }, { id: "draw", label: "Allow attacker to draw 1 card" }, { id: 4, label: "bad" }], eligibleHandKeys: ["hand:0", 4] } }], requirement: "dodge", options: [{ providerId: "card", satisfies: "dodge", label: "Play Dodge", selection: { type: "cards", min: 1, max: 1, eligibleCardIds: ["dodge-1", 9] } }, { providerId: 9, satisfies: "dodge", label: "Invalid", selection: null }] },
  });
  ...
  assert.deepEqual(room?.currentAction?.triggerOptions, [{ effectId: "yin_yang_swords_attack_targeted", label: "Yin-Yang Swords", allowDecline: false, timeoutChoiceId: "draw", selection: { type: "choice", choices: [{ id: "discard", label: "Discard 1 hand card" }, { id: "draw", label: "Allow attacker to draw 1 card" }], eligibleHandKeys: ["hand:0"] } }]);
});
