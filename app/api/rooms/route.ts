...
function parse<T>(value: string | null, fallback: T): T {
  try { return value ? asLegacyResponsePending(JSON.parse(value)) as T : fallback; } catch { return fallback; }
}
...
async function beginAttackTargeted(room: RoomRow, declaration: AttackDeclaration, source: PlayerRow, target: PlayerRow, discard: Card[], log: string[], eventId: string, writes: D1PreparedStatement[] = []) {
  ...
  if (isBotPlayer(target)) await advanceCanonicalBotTrigger(room.id);
  return true;
}
...
async function startJudgementNegation(room: RoomRow, target: PlayerRow, players: PlayerRow[], delayed: Card, deck: Card[], discard: Card[], log: string[]) {
  ...
  const pending: NegationPending = withPresentationBarrier({ kind: "negation", sourceId: target.id, actorId: holders[0].id, remainingIds: holders.slice(1).map((player) => player.id), negated: false, cardName: cardDefinition(delayed.kind).name, effectTargetId: target.id, resumePhase: room.phase?.startsWith("draw") ? room.phase : "draw", effect: { kind: "judgement", targetId: target.id, cardId: delayed.id }, responseTarget: `${cardDefinition(delayed.kind).name}'s effect on ${target.name}`, chainDepth: 0, reason: `Play Negation to cancel ${cardDefinition(delayed.kind).name}'s effect on ${target.name}, or pass`, deadline: nextResponseDeadline(holders[0]) }, presentation.eventId);
...
async function resolveDeferredStratagem(roomId: string, pending: NegationPending): Promise<Card[]> {
  ...
  if (pending.effect.kind === "draw_two") {
    ...
    await db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, log_json = ? WHERE id = ?").bind(serializePending(group), JSON.stringify(log), roomId).run();
...
async function advanceNegation(roomId: string) {
  for (let guard = 0; guard < 16; guard++) {
    const room = await db().prepare("SELECT * FROM rooms WHERE id = ?").bind(roomId).first<RoomRow>(); const stored = parse<Pending | null>(room?.pending_json ?? null, null); const pending = asLegacyResponsePending(stored) as NegationPending | null;
    ...
    const decision = responseDecisionFor(asResponsePending(pending), responseContext(actor));
    ...
    if (option && !isBotPlayer(actor)) return;
    ...
    const botWillNegate = Boolean(execution?.status === "satisfied" && !pending.negated && actor.id === pending.effectTargetId);
    ...
    if (!botWillNegate) {
      ...
      log = addLog(log, `${actor.name} passes the Negation opportunity for ${pending.responseTarget ?? pending.cardName}.`);
      const nextId = pending.remainingIds[0];
      if (nextId) { const nextActor = await db().prepare("SELECT * FROM players WHERE id = ?").bind(nextId).first<PlayerRow>(); const next: NegationPending = { ...pending, actorId: nextId, remainingIds: pending.remainingIds.slice(1), deadline: nextResponseDeadline(nextActor) }; await db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, log_json = ? WHERE id = ?").bind(serializePending(next), JSON.stringify(log), roomId).run(); continue; }
      log = addLog(log, `Negation window closes for ${pending.responseTarget ?? pending.cardName}; resolving the effect.`);
      await db().prepare("UPDATE rooms SET phase = 'resolving', log_json = ? WHERE id = ?").bind(JSON.stringify(log), roomId).run();
      await resolveDeferredStratagem(roomId, pending); return;
    }
    if (!execution || execution.status !== "satisfied") return;
    const consumed = execution.consumeCardIds ?? [];
    const consumedCards = consumed.map((id) => hand.find((card) => card.id === id)).filter((card): card is Card => Boolean(card));
    if (consumedCards.length !== consumed.length) return;
    const discard = parse<Card[]>(room.discard_json, []); let log = parse<string[]>(room.log_json, []); const nextHand = hand.filter((card) => !consumed.includes(card.id));
    ...
    const readyAfterEventId = latestDecisionPresentationEventId(log, pending.resolutionId);
...
async function startNegation(room: RoomRow, source: PlayerRow, players: PlayerRow[], card: Card, targetName: string, effectTargetId: string, effect: DeferredStratagem, hand: Card[], deck: Card[], discard: Card[], log: string[]): Promise<Card[]> {
  ...
  const pending: NegationPending = readyAfterEventId
    ? withPresentationBarrier({ kind: "negation", ...base, actorId: holders[0].id, remainingIds: holders.slice(1).map((player) => player.id), reason: `Play Negation to cancel ${cardDefinition(card.kind).name}'s effect on ${targetName}, or pass`, deadline: nextResponseDeadline(holders[0]) }, log, readyAfterEventId)
    : { kind: "negation", ...base, actorId: holders[0].id, remainingIds: holders.slice(1).map((player) => player.id), reason: `Play Negation to cancel ${cardDefinition(card.kind).name}'s effect on ${targetName}, or pass`, deadline: nextResponseDeadline(holders[0]) };
...
async function resolveAttackDamageAboutToApply({ room, source, target, players, sourceHand, discard, log, resumePhase, sequenceStartCardId, label = "Attack", writes = [], onDamageApplied, skipTriggers = false }: AttackDamageTransition): Promise<AttackDamageResult> {
  ...
  if (!skipTriggers && options.length) {
    ...
    if (isBotPlayer(source)) await advanceCanonicalBotTrigger(room.id);
  ...
  else await continueAfterDying(room.id, source.id);
  return { kind: "damage_applied", hp, log: damageLog };
}

/** Runs the same live provider discovery and semantic outcome path for bots. */
async function advanceCanonicalBotTrigger(roomId: string) {
  const room = await db().prepare("SELECT * FROM rooms WHERE id = ?").bind(roomId).first<RoomRow>();
  const pending = asTriggerPending(parse<Pending | null>(room?.pending_json ?? null, null));
  ...
  const context = triggerContextFor(pending, players); const options = context ? triggerOptionsFor(pending, players) : [];
  const choice = chooseBotTrigger(options);
  const option = choice ? options.find((candidate) => candidate.effectId === choice.providerId) : undefined;
  if (!context || !option) {
  ...
  if (!execution) return;
  const sourceId = pending.continuation.kind === "attack_targeted_event" ? pending.continuation.declaration.sourceId : pending.continuation.sourceId;
  const targetId = pending.continuation.kind === "attack_targeted_event" ? pending.continuation.declaration.targetId : pending.continuation.targetId;
  const source = players.find((player) => player.id === sourceId && player.alive);
  ...
  if (execution.outcome.kind === "follow_up_attack") {
    ...
    if (next && remaining.length) {
      const presentation = freshDecision(next, log, `${actor.name} resolves an optional reaction. Another reaction remains available.`);
      await db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, log_json = ? WHERE id = ?").bind(serializePending(presentation.pending), JSON.stringify(presentation.log), roomId).run();
    }
    else await resumeCanonicalTriggerContinuation(room, pending.continuation as AttackDodgedTriggerContinuation | DamageAboutToApplyTriggerContinuation, players, discard, log);
  }
}
...
async function finishDodgedAttack(room: RoomRow, source: PlayerRow | null, target: PlayerRow | null, discard: Card[], log: string[], resumePhase: string, sequenceStartCardId: string, writes: D1PreparedStatement[] = []) {
  ...
  if (source?.alive && target?.alive && options.length) {
    if (isBotPlayer(source)) {
      ...
      const pending: TriggerPending = { kind: "trigger", event: "attack_dodged", actorId: source.id, resolutionId, reason: `${source.name}'s Attack is blocked. An optional reaction may apply.`, deadline: nextResponseDeadline(source), continuation: { kind: "attack_dodged_event", sourceId: source.id, targetId: target.id, resumePhase, sequenceStartCardId, resolutionId }, ...(readyAfterEventId ? { readyAfterEventId } : {}) };
...
async function resumeCanonicalTriggerContinuation(room: RoomRow, continuation: AttackTargetedTriggerContinuation | AttackDodgedTriggerContinuation | DamageAboutToApplyTriggerContinuation, players: PlayerRow[], discard: Card[], log: string[]) {
  if (continuation.kind === "attack_targeted_event") {
    ...
    if (continuation.group) {
      await beginGroupTarget(room, continuation.group, players, discard, nextLog);
      return;
    }
    if (isNioShieldImmune(target, attackPhysicalCard(declaration), source)) {
      ...
      await continueAfterDying(room.id, source.id); return;
    }
    ...
    if (!isBotPlayer(target) && canRespondWithDodge(target)) {
      ...
      await db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, log_json = ? WHERE id = ?").bind(serializePending(withPresentationBarrier(attackPending(declaration, target), presentation.log, presentation.eventId)), JSON.stringify(presentation.log), room.id).run(); return;
    }
    if (dodge) {
      ...
      await finishDodgedAttack(room, source, { ...target, hand_json: JSON.stringify(nextHand) }, discard, nextLog, declaration.resumePhase, declaration.sequenceStartCardId, [db().prepare("UPDATE players SET hand_json = ? WHERE id = ?").bind(JSON.stringify(nextHand), target.id)]); return;
    }
    await resolveAttackDamageAboutToApply({ room, source, target, players, sourceHand: parse<Card[]>(source.hand_json, []), discard, log: nextLog, resumePhase: declaration.resumePhase, sequenceStartCardId: declaration.sequenceStartCardId, label: "Attack", skipTriggers: false });
    return;
  }
...
async function advanceFrostSword(roomId: string) {
  ...
  const pending = asLegacyTriggerPending(stored) as FrostSwordPending | null;
...
async function advanceRockCleaving(roomId: string) {
  ...
  const pending = asLegacyTriggerPending(stored) as RockCleavingPending | null;
...
async function applyFollowUpAttackOutcome(room: RoomRow, continuation: AttackDodgedTriggerContinuation, source: PlayerRow, target: PlayerRow, players: PlayerRow[], attack: Card, sourceHand: Card[], discard: Card[], log: string[], presentationLabel = "optional reaction") {
  ...
  if (!isBotPlayer(target) && canRespondWithDodge(target)) {
    ...
    const attackPendingState = withPresentationBarrier(attackPending(declaration, target), presentation.log, followUpPresentation.eventId);
    await db().batch([
      db().prepare("UPDATE players SET hand_json = ? WHERE id = ?").bind(JSON.stringify(nextSourceHand), source.id),
      db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, discard_json = ?, log_json = ? WHERE id = ?").bind(serializePending(attackPendingState), JSON.stringify(discard), JSON.stringify(presentation.log), room.id),
    ]);
    return;
  }
  if (dodge) {
...
async function advanceGreenDragon(roomId: string) {
  const room = await db().prepare("SELECT * FROM rooms WHERE id = ?").bind(roomId).first<RoomRow>();
  const stored = parse<Pending | null>(room?.pending_json ?? null, null); const trigger = asTriggerPending(stored);
  const pending = asLegacyTriggerPending(stored) as GreenDragonPending | null;
...
async function advanceDuel(roomId: string) {
  for (let guard = 0; guard < 30; guard++) {
    ...
    const pending = asLegacyResponsePending(stored) as DuelPending | null;
...
function nextGroupPending(pending: GroupPending, players: PlayerRow[]) {
  ...
  return { ...pending, readyAfterEventId: undefined, actorId, remainingIds: nextIds.slice(1), reason: `Respond to ${groupCardName(pending.cardKind)}: select ${pending.requiredKind} or take 1 damage`, deadline: nextResponseDeadline(actor) } satisfies GroupPending;
}
...
async function finishGroupStep(room: RoomRow, pending: GroupPending, players: PlayerRow[], discard: Card[], log: string[], writes: D1PreparedStatement[] = []) {
  ...
  if (next) {
    ...
    const readyAfterEventId = latestDecisionPresentationEventId(presentation.log, pending.resolutionId);
    await beginGroupTarget(room, readyAfterEventId ? withPresentationBarrier(next, presentation.log, readyAfterEventId) : { ...next, readyAfterEventId: undefined }, players, discard, presentation.log, writes);
    return;
  }
...
async function advanceGroup(roomId: string) {
  for (let guard = 0; guard < 30; guard++) {
    const room = await db().prepare("SELECT * FROM rooms WHERE id = ?").bind(roomId).first<RoomRow>();
    const stored = parse<Pending | null>(room?.pending_json ?? null, null);
    const pending = asLegacyResponsePending(stored) as GroupPending | null;
...
async function advanceHarvest(roomId: string) {
  for (let guard = 0; guard < 12; guard++) {
    ...
    if (!pending.previewCardId) {
      ...
      await db().prepare("UPDATE rooms SET pending_json = ? WHERE id = ? AND phase = 'response' AND pending_json = ?").bind(serializePending(preview), roomId, room.pending_json).run();
      return;
    }
    if ((pending.botAdvanceAt ?? 0) > Date.now()) return;
...
async function runBots(roomId: string) {
  for (let guard = 0; guard < 12; guard++) {
    ...
    if (room.phase?.startsWith("draw")) {
      ...
      else { const draw = drawCards(deck, discard, 2, log); deck = draw.deck; discard = draw.discard; log = addLog(draw.log, `${bot.name}'s turn started · drawing ${draw.drawn.length} card${draw.drawn.length === 1 ? "" : "s"}.`, bot.id); hand.push(...draw.drawn); }
    ...
    for (let drawTwo = hand.find((card) => card.kind === "DrawTwo"); drawTwo; drawTwo = hand.find((card) => card.kind === "DrawTwo")) {
      ...
      log = addCardEvent(draw.log, bot.name, drawTwo); log = addHistory(log, `${bot.name} plays Something Out of Nothing and draws ${draw.drawn.length} cards.`, bot.id);
    }
    ...
    if (oath && wounded.length) {
      ...
      log = addCardEvent(log, bot.name, oath, "All wounded players"); log = addFinalResult(log, `${bot.name} plays Oath of the Peach Garden. ${wounded.map((player) => player.name).join(", ")} recover 1 HP.`);
    }
    const steal = hand.find((card) => card.kind === "Steal");
    const stealTarget = players.find((player) => player.alive && player.id !== bot.id && attackDistance(players, bot.id, player.id) <= 1 && parse<Card[]>(player.hand_json, []).length > 0);
    ...
    if (groupCard && groupTargets.length) {
      ...
      const groupPresentation = addCardEventWithId(log, bot.name, groupCard, "All other players"); log = addLog(groupPresentation.log, `${bot.name} plays ${cardName}. Action passes to ${groupTargets[0].name} to play ${requiredKind}.`);
        const pending: GroupPending = withPresentationBarrier({ kind: "group", cardKind: groupCard.kind, sourceId: bot.id, actorId: groupTargets[0].id, remainingIds: groupTargets.slice(1).map((player) => player.id), requiredKind, resumePhase: "play", reason: `Respond to ${cardName}: select ${requiredKind} or take 1 damage`, deadline: nextResponseDeadline(groupTargets[0]), heldCards: [groupCard], resolutionId: latestResolutionId(log) }, log, groupPresentation.eventId);
      writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
    ...
    if (duel && duelTarget) {
      ...
      const duelPresentation = addCardEventWithId(log, bot.name, duel, duelTarget.name); log = addLog(duelPresentation.log, `${bot.name} starts a Duel with ${duelTarget.name}. Action passes to ${duelTarget.name} to play Attack.`);
      const pending: DuelPending = withPresentationBarrier({ kind: "duel", sourceId: bot.id, targetId: duelTarget.id, actorId: duelTarget.id, opponentId: bot.id, resumePhase: "play", reason: "Respond to Duel: select Attack or take 1 damage", deadline: nextResponseDeadline(duelTarget) }, log, duelPresentation.eventId);
      writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
      await db().batch(writes);
      const playersForNegation = players.map((player) => player.id === bot.id ? { ...player, hand_json: JSON.stringify(hand) } : changedHands.has(player.id) ? { ...player, hand_json: JSON.stringify(changedHands.get(player.id)) } : player);
    ...
    if (attackCards.length && target) {
      ...
      if (!(halberdTargets.length > 1 && attack)) discard.push(...attackCards);
      if (halberdTargets.length > 1 && attack) {
        const halberdPresentation = addCardEventWithId(log, bot.name, attack, halberdTargets.map((player) => player.name).join(", ")); log = halberdPresentation.log;
        log = addLog(log, `${bot.name} uses their last hand card as Attack with Sky Piercing Halberd, targeting ${halberdTargets.map((player) => player.name).join(", ")}. ${halberdTargets[0].name} resolves first.`);
        const pending: GroupPending = withPresentationBarrier({ kind: "group", cardKind: "SkyPiercingHalberdAttack", sourceId: bot.id, actorId: halberdTargets[0].id, remainingIds: halberdTargets.slice(1).map((player) => player.id), requiredKind: "Dodge", resumePhase: phaseAfterAttack(bot), reason: "Respond to Sky Piercing Halberd Attack: select Dodge or take 1 damage", deadline: nextResponseDeadline(halberdTargets[0]), heldCards: [attack], resolutionId: latestResolutionId(log) }, log, halberdPresentation.eventId);
        writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
      ...
      if (isNioShieldImmune(target, attackPhysicalCard(declaration), bot)) {
        ...
        const damageResult = await resolveAttackDamageAboutToApply({
          room, source: bot, target, players, sourceHand: hand, discard, log,
          resumePhase: phaseAfterAttack(bot), sequenceStartCardId,
          label: attack ? "Attack" : "Serpent Spear Attack", writes,
          onDamageApplied: (hp) => { target.hp = hp; },
        });
        if (damageResult.kind === "reaction_pending" || damageResult.kind === "dying") return;
...
function legalActionsFor(room: RoomRow, actor: PlayerRow | undefined, pending: Pending | null, players: PlayerRow[]): GameplayAction[] {
  ...
  if (room.phase === "response") {
    ...
    switch (pending.kind) {
      case "borrowed_sword": return pending.stage === "choose_target" ? ["choose_borrowed_sword_target"] : ["decline_borrowed_sword", ...(hand.some(isAttackCard) ? ["respond_borrowed_sword"] : [])];
...
export async function POST(request: Request) {
  ...
  if (GAMEPLAY_ACTION_SET.has(action)) {
    ...
    // canonical ResponsePending wrapper.
    const contextMismatch = submitted && ((!acceptsLegacyPendingKind && submitted.actionRevision !== undefined && String(submitted.actionRevision) !== expectedRevision) || submitted.meId !== undefined && String(submitted.meId) !== String(expectedContext.meId) || submitted.phase !== undefined && String(submitted.phase) !== String(expectedContext.phase) || submittedPendingKind !== undefined && submittedPendingKind !== String(expectedContext.pendingKind) && !acceptsLegacyPendingKind || submitted.actorId !== undefined && String(submitted.actorId) !== String(expectedContext.actorId));
    if (contextMismatch) {
    ...
    if (action === "respond") {
      ...
      if (!responseExecution && submittedAction !== action && me) {
        const response = asResponsePending(pendingForController);
        const kind = response?.requirement.kind === "dodge" ? "Dodge" : response?.requirement.kind === "attack" ? "Attack" : null;
        const selected = kind ? selectResponse(responseContext(me), kind, body.cardId, body.cardIds) : undefined;
        if (selected?.cards?.length) responseExecution = resolveResponseDecision(pendingForController, responseContext(me), selected.providerId, { cardId: selected.cards[0].id, cardIds: selected.cards.map((card) => card.id) });
      }
  ...
  if (canonicalResponseKind === "negation" && (canonicalResponseSatisfied || canonicalResponseDeclined)) {
    ...
    if (canonicalRespond) {
      ...
      const presentation = addLogWithId(parse<string[]>(liveRoom.log_json, []), `${me.name} passes the Negation opportunity for ${pending.responseTarget ?? pending.cardName}.`);
      ...
      await db.prepare("UPDATE rooms SET phase = 'response', pending_json = ?, log_json = ? WHERE id = ?").bind(serializePending({ ...next, readyAfterEventId: undefined }), JSON.stringify(log), room.id).run(); await advanceNegation(room.id);
    } else {
      let log = addLog(parse<string[]>(liveRoom.log_json, []), `${me.name} passes the Negation opportunity for ${pending.responseTarget ?? pending.cardName}.`);
      log = addLog(log, `Negation window closes for ${pending.responseTarget ?? pending.cardName}; resolving the effect.`);
  ...
  if (action === "choose_borrowed_sword_target") {
    ...
    const holderHand = parse<Card[]>(holder.hand_json, []); const attack = holderHand.find(isAttackCard); let log = parse<string[]>(liveRoom.log_json, []);
    if (!attack) {
      const weapon = equipmentZone(holder).weapon; const sourceHand = [...parse<Card[]>(me.hand_json, []), ...(weapon ? [weapon] : [])]; const equipment = equipmentZone(holder); delete equipment.weapon;
  ...
  if (action === "respond_borrowed_sword" || action === "decline_borrowed_sword") {
    ...
    const transfer = async () => { const weapon = equipmentZone(weaponHolder).weapon; if (!weapon) return; const equipment = equipmentZone(weaponHolder); delete equipment.weapon; const sourceHand = [...parse<Card[]>(source.hand_json, []), weapon]; const log = addLog(parse<string[]>(liveRoom.log_json, []), `${source.name} obtains ${cardDefinition(weapon.kind).name} from ${weaponHolder.name} after the forced Attack fails.`); await db.batch([db.prepare("UPDATE players SET equipment_json = ? WHERE id = ?").bind(JSON.stringify(equipment), weaponHolder.id), db.prepare("UPDATE players SET hand_json = ? WHERE id = ?").bind(JSON.stringify(sourceHand), source.id), db.prepare("UPDATE rooms SET phase = ?, pending_json = NULL, log_json = ? WHERE id = ?").bind(pending.resumePhase, JSON.stringify(log), room.id)]); };
    ...
    const attack = parse<Card[]>(me.hand_json, []).find((card) => card.id === String(body.cardId ?? "") && isAttackCard(card)); if (!attack || !target || attackDistance(players, me.id, target.id) > attackRangeFor(me)) return json({ error: "Choose a legal Attack against the selected target." }, 400);
    ...
    const hand = parse<Card[]>(me.hand_json, []).filter((card) => card.id !== attack.id); const discard = [...parse<Card[]>(liveRoom.discard_json, []), attack]; const presentation = addCardEventWithId(parse<string[]>(liveRoom.log_json, []), me.name, attack, target.name); const declaration = attackDeclaration(me, target, "triggered", [attack], pending.resumePhase, attack); const next = attackPending(declaration, target);
    ...
    if (!targetHand.some((item) => item.kind === "Dodge") && !isNioShieldImmune(target, attack, me)) {
      ...
      await resolveAttackDamageAboutToApply({ room: liveRoom, source: { ...me, hand_json: JSON.stringify(hand) }, target, players, sourceHand: hand, discard, log: presentation.log, resumePhase: pending.resumePhase, sequenceStartCardId: pending.weaponId ?? attack.id });
    } else {
  ...
  if (canonicalResponseKind === "attack" && (canonicalResponseSatisfied || canonicalResponseDeclined)) {
    ...
    if (canonicalDodge) {
      ...
      if (!source?.alive) {
        await db.prepare("UPDATE rooms SET phase = ?, pending_json = NULL, log_json = ? WHERE id = ?").bind(pending.resumePhase ?? "play", JSON.stringify(addLog(log, "The Attack source is no longer available; the response ends.")), room.id).run();
      } else {
        const rows = await db.prepare("SELECT * FROM players WHERE room_id = ? ORDER BY seat").bind(room.id).all<PlayerRow>();
        await resolveAttackDamageAboutToApply({ room: liveRoom, source, target: me, players: rows.results ?? [], sourceHand: parse<Card[]>(source.hand_json, []), discard, log, resumePhase: pending.resumePhase ?? phaseAfterAttack(source), sequenceStartCardId: pending.sequenceStartCardId ?? "", label: "Attack" });
      }
    }
    return json({ room: await roomState(code, token) });
  }
  ...
  if (["draw", "play_card", "serpent_spear_attack", "end_turn"].includes(action)) {
    ...
    if (action === "draw") {
      ...
      if (!isBotPlayer(target) && canRespondWithDodge(target)) {
        ...
        const nextPending = readyAfterEventId ? withPresentationBarrier(attackPending(declaration, target), presentation.log, readyAfterEventId) : attackPending(declaration, target);
        ...
        await resolveAttackDamageAboutToApply({ room: liveRoom, source: me, target, players, sourceHand: hand, discard, log, resumePhase: phaseAfterAttack(me), sequenceStartCardId: declaration.sequenceStartCardId, label: "Serpent Spear Attack" });
      }
      ...
      if (card.kind === "Peach") {
        ...
        if (!isBotPlayer(target) && canRespondWithDodge(target)) {
          ...
          await db.batch([db.prepare("UPDATE players SET hand_json = ? WHERE id = ?").bind(JSON.stringify(hand), me.id), db.prepare("UPDATE rooms SET phase = 'response', pending_json = ?, discard_json = ?, log_json = ? WHERE id = ?").bind(serializePending(withPresentationBarrier(attackPending(declaration, target), presentation.log, attackPresentation.eventId)), JSON.stringify(discard), JSON.stringify(presentation.log), room.id)]);
        } else if (dodge) {
          ...
          await resolveAttackDamageAboutToApply({ room: liveRoom, source: me, target, players: rows.results ?? [], sourceHand: hand, discard, log, resumePhase: phaseAfterAttack(me), sequenceStartCardId: card.id });
        }
      } else {
