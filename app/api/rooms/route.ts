...
async function advanceNegation(roomId: string) {
  for (let guard = 0; guard < 16; guard++) {
    ...
    const rows = await db().prepare("SELECT * FROM players WHERE room_id = ? ORDER BY seat").bind(roomId).all<PlayerRow>(); const updatedPlayers = (rows.results ?? []).map((player) => player.id === actor.id ? { ...player, hand_json: JSON.stringify(nextHand) } : player); const holders = playersHoldingNegation(updatedPlayers, actor.seat);
...
async function startNegation(room: RoomRow, source: PlayerRow, players: PlayerRow[], card: Card, targetName: string, effectTargetId: string, effect: DeferredStratagem, hand: Card[], deck: Card[], discard: Card[], log: string[]): Promise<Card[]> {
  ...
  const holders = playersHoldingNegation(players.map((player) => player.id === source.id ? { ...player, hand_json: JSON.stringify(hand) } : player), source.seat);
  ...
  log = addLog(log, `Negation window opens for ${base.responseTarget}.`);
  if (!holders.length) {
  ...
  const pending: NegationPending = withPresentationBarrier({ kind: "negation", ...base, actorId: holders[0].id, remainingIds: holders.slice(1).map((player) => player.id), reason: `Play Negation to cancel ${cardDefinition(card.kind).name}'s effect on ${targetName}, or pass`, deadline: nextResponseDeadline(holders[0]) }, log);
...
async function finishDodgedAttack(room: RoomRow, source: PlayerRow | null, target: PlayerRow | null, discard: Card[], log: string[], resumePhase: string, sequenceStartCardId: string, writes: D1PreparedStatement[] = []) {
  ...
  if (source?.alive && target?.alive && options.length) {
    // Bot advancement still consumes the old saved-continuation adapters while
    // its scheduler is migrated. Human and Quick Test decisions below are
    // fully event-shaped.
    if (isBotPlayer(source)) {
      const option = options[0];
      log = addLog(log, `${source.name}'s Attack is blocked. ${option.label.replace(/^Use\s+/, "")} may continue.`);
      if (option.effectId === "green_dragon_blade_attack_dodged") {
        const legacy: GreenDragonPending = { kind: "green_dragon", triggerId: option.effectId, sourceId: source.id, targetId: target.id, actorId: source.id, resumePhase, sequenceStartCardId, reason: option.label, deadline: nextResponseDeadline(source) };
        writes.push(db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, discard_json = ?, log_json = ? WHERE id = ?").bind(serializePending(withPresentationBarrier(legacy, log)), JSON.stringify(discard), JSON.stringify(log), room.id));
        await db().batch(writes); await advanceGreenDragon(room.id); return;
      }
      if (option.effectId === "rock_cleaving_axe_attack_dodged") {
        const legacy: RockCleavingPending = { kind: "rock_cleaving", triggerId: option.effectId, sourceId: source.id, targetId: target.id, actorId: source.id, resumePhase, sequenceStartCardId, reason: option.label, deadline: nextResponseDeadline(source) };
        writes.push(db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, discard_json = ?, log_json = ? WHERE id = ?").bind(serializePending(withPresentationBarrier(legacy, log)), JSON.stringify(discard), JSON.stringify(log), room.id));
        await db().batch(writes); await advanceRockCleaving(room.id); return;
      }
    }
...
async function applyForcedDamageOutcome(room: RoomRow, continuation: AttackDodgedTriggerContinuation, source: PlayerRow, target: PlayerRow, players: PlayerRow[], materials: Card[], hand: Card[], discard: Card[], log: string[]) {
...
async function applyFollowUpAttackOutcome(room: RoomRow, continuation: AttackDodgedTriggerContinuation, source: PlayerRow, target: PlayerRow, players: PlayerRow[], attack: Card, sourceHand: Card[], discard: Card[], log: string[]) {
  ...
  const declaration = { ...attackDeclaration(source, target, "green_dragon", [attack], continuation.resumePhase, attack), sequenceStartCardId: continuation.sequenceStartCardId } satisfies AttackDeclaration;
  ...
  if (!isBotPlayer(target) && canRespondWithDodge(target)) {
    const attackPendingState = withPresentationBarrier(attackPending(declaration, target), log);
    await db().batch([
      db().prepare("UPDATE players SET hand_json = ? WHERE id = ?").bind(JSON.stringify(nextSourceHand), source.id),
      db().prepare("UPDATE rooms SET phase = 'response', pending_json = ?, discard_json = ?, log_json = ? WHERE id = ?").bind(serializePending(attackPendingState), JSON.stringify(discard), JSON.stringify(log), room.id),
    ]);
    return;
  }
  if (dodge) {
...
async function advanceDuel(roomId: string) {
  for (let guard = 0; guard < 30; guard++) {
    ...
    const nextPending: DuelPending = withPresentationBarrier({ ...pending, readyAfterEventId: undefined, actorId: opponent.id, opponentId: actor.id, reason: `Respond to Duel: select Attack or take 1 damage`, deadline: nextResponseDeadline(opponent) }, log);
    await db().batch([
  ...
  for (let guard = 0; guard < 12; guard++) {
    ...
    if (groupCard && groupTargets.length) {
      ...
      log = addCardEvent(log, bot.name, groupCard, "All other players"); log = addLog(log, `${bot.name} plays ${cardName}. Action passes to ${groupTargets[0].name} to play ${requiredKind}.`);
        const pending: GroupPending = withPresentationBarrier({ kind: "group", cardKind: groupCard.kind, sourceId: bot.id, actorId: groupTargets[0].id, remainingIds: groupTargets.slice(1).map((player) => player.id), requiredKind, resumePhase: "play", reason: `Respond to ${cardName}: select ${requiredKind} or take 1 damage`, deadline: nextResponseDeadline(groupTargets[0]), heldCards: [groupCard], resolutionId: latestResolutionId(log) }, log);
      writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
    ...
    if (duel && duelTarget) {
      ...
      const pending: DuelPending = withPresentationBarrier({ kind: "duel", sourceId: bot.id, targetId: duelTarget.id, actorId: duelTarget.id, opponentId: bot.id, resumePhase: "play", reason: "Respond to Duel: select Attack or take 1 damage", deadline: nextResponseDeadline(duelTarget) }, log);
      writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
      await db().batch(writes);
      const playersForNegation = players.map((player) => player.id === bot.id ? { ...player, hand_json: JSON.stringify(hand) } : changedHands.has(player.id) ? { ...player, hand_json: JSON.stringify(changedHands.get(player.id)) } : player);
    ...
    if (attackCards.length && target) {
      ...
      if (halberdTargets.length > 1 && attack) {
        log = addCardEvent(log, bot.name, attack, halberdTargets.map((player) => player.name).join(", "));
        ...
        const pending: GroupPending = withPresentationBarrier({ kind: "group", cardKind: "SkyPiercingHalberdAttack", sourceId: bot.id, actorId: halberdTargets[0].id, remainingIds: halberdTargets.slice(1).map((player) => player.id), requiredKind: "Dodge", resumePhase: phaseAfterAttack(bot), reason: "Respond to Sky Piercing Halberd Attack: select Dodge or take 1 damage", deadline: nextResponseDeadline(halberdTargets[0]), heldCards: [attack], resolutionId: latestResolutionId(log) }, log);
        writes.push(db().prepare("UPDATE players SET hand_json = ?, hp = ? WHERE id = ?").bind(JSON.stringify(hand), bot.hp, bot.id));
...
export async function POST(request: Request) {
  ...
  if (GAMEPLAY_ACTION_SET.has(action)) {
    ...
    if (action === "decline_response") {
      ...
      action = response.continuation.kind === "negation" ? "pass_negation"
        : response.continuation.kind === "attack" ? "take_damage"
  ...
  if (action === "apply_trigger" && triggerExecution?.outcome === "continue_event") {
    ...
    if (remaining.length) {
      ...
      const resumePhase = next.continuation.resumePhase;
      await db.prepare("UPDATE rooms SET phase = ?, pending_json = NULL, log_json = ? WHERE id = ?").bind(resumePhase, JSON.stringify(log), room.id).run();
      const source = players.find((player) => player.id === next.continuation.sourceId);
      if (source) await continueAfterDying(room.id, source.id);
    }
    return json({ room: await roomState(code, token) });
  }
  ...
  if (["apply_trigger", "decline_trigger_effect"].includes(action)) {
    ...
    if (liveRoom && trigger && continuation?.kind === "attack_dodged_event" && trigger.actorId === me.id) {
      ...
      if (!source || !target || action === "decline_trigger_effect") {
        ...
        await db.prepare("UPDATE rooms SET phase = ?, pending_json = NULL, discard_json = ?, log_json = ? WHERE id = ?").bind(continuation.resumePhase, JSON.stringify(discard), JSON.stringify(nextLog), room.id).run();
        if (source) await continueAfterDying(room.id, source.id);
        return json({ room: await roomState(code, token) });
      }
      if (triggerExecution?.outcome === "follow_up_attack") {
        const attack = triggerExecution.consumeCardIds?.length === 1 ? hand.find((card) => card.id === triggerExecution.consumeCardIds?.[0]) ?? null : null;
        ...
        await applyFollowUpAttackOutcome(liveRoom, continuation, source, target, players, attack, hand, discard, log);
        return json({ room: await roomState(code, token) });
      }
      if (triggerExecution?.outcome === "force_damage") {
        const cards = rockCleavingCards(source, hand);
        const materials = triggerExecution.consumeCardIds?.map((id) => cards.find((card) => card.id === id)).filter((card): card is Card => Boolean(card)) ?? [];
        if (materials.length !== 2) return json({ error: "The selected reaction no longer has its required costs.", stale: true, room: await roomState(code, token) }, 409);
        await applyForcedDamageOutcome(liveRoom, continuation, source, target, players, materials, hand, discard, log);
        return json({ room: await roomState(code, token) });
      }
  ...
  if (["respond_negation", "pass_negation"].includes(action)) {
    ...
    if (negation) {
      ...
      const rows = await db.prepare("SELECT * FROM players WHERE room_id = ? ORDER BY seat").bind(room.id).all<PlayerRow>(); const updatedPlayers = (rows.results ?? []).map((player) => player.id === me.id ? { ...player, hand_json: JSON.stringify(hand) } : player); const holders = playersHoldingNegation(updatedPlayers, me.seat);
  ...
  if (["respond_duel", "take_duel_damage"].includes(action)) {
    ...
    if (!selectedAttack && !serpentCards.length) {
      ...
      const nextPending: DuelPending = withPresentationBarrier({ ...pending, readyAfterEventId: undefined, actorId: opponent.id, opponentId: me.id, reason: "Respond to Duel: select Attack or take 1 damage", deadline: nextResponseDeadline(opponent) }, log);
      await db.batch([
  ...
  if (action === "respond_eight_trigrams") {
    if (!me) return json({ error: "Your player session is no longer valid." }, 403);
    const liveRoom = await db.prepare("SELECT * FROM rooms WHERE id = ?").bind(room.id).first<RoomRow>(); const stored = parse<Pending | null>(liveRoom?.pending_json ?? null, null); const pending = asLegacyResponsePending(stored) as AttackPending | GroupPending | null;
  ...
  if (["draw", "play_card", "serpent_spear_attack", "end_turn"].includes(action)) {
    ...
    if (action === "draw") {
      ...
      if (card.kind === "Peach") {
        ...
        const pending: DuelPending = withPresentationBarrier({ kind: "duel", sourceId: me.id, targetId: target.id, actorId: target.id, opponentId: me.id, resumePhase: liveRoom.phase, reason: "Respond to Duel: select Attack or take 1 damage", deadline: nextResponseDeadline(target) }, log);
        const rows = await db.prepare("SELECT * FROM players WHERE room_id = ? ORDER BY seat").bind(room.id).all<PlayerRow>();
        ...
        log = addCardEvent(log, me.name, card, targets.map((entry) => entry.name).join(", "));
