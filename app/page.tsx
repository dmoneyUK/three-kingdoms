...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const canPlayAsAttack = (item: Card) => isAttackCard(item) || playPhaseAttackCardIds.has(item.id);
  const selectedCanPlayAsAttack = Boolean(card && canPlayAsAttack(card));
  const halberdAttack = Boolean(selectedCanPlayAsAttack && me?.equipmentCards.some((equipment) => equipment.kind === "SkyPiercingHalberd") && room.myHand.length === 1);
