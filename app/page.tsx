...
function CardFace({ card }: { card: Card }) {
  ...
  return <div className={`played-card ${card.kind.toLowerCase()} ${suitColorClass(card.suit)}`}><i>{card.rank}<small>{card.suit}</small></i><span className="suit-watermark" aria-hidden="true">{card.suit}</span><b className="card-name-mark">{definition.name}</b><strong>{definition.category} card</strong></div>;
}
