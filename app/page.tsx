...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const [selected, setSelected] = useState(""); const [playAsAttack, setPlayAsAttack] = useState(false); const [targetIds, setTargetIds] = useState<string[]>([]); const target = targetIds[0] ?? "";
  ...
  useEffect(() => { const timer = setTimeout(() => { setSelected(""); setPlayAsAttack(false); setTargetIds([]); setTargetCardIndex(null); setTargetCardZone(""); setTargetCardId(""); setDiscardSelected([]); setSerpentMode(false); setSerpentSelected([]); setResponseProviderId(""); }, 0); return () => clearTimeout(timer); }, [room.turnSeat, room.phase, room.meId]);
