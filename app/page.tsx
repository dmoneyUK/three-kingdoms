...
export function LocalPlayerDock({ player, hero, children, onHeroInfo, onInfoCard, equipmentSelection = null, hiddenCardIds = new Set() }: LocalPlayerDockProps) {
  ...
  const renderZoneCard = (card: Card, selected = false, selectable = false) => <span className={`local-zone-card ${suitColorClass(card.suit)} ${selected ? "selected-cost" : ""}`} key={card.id} data-equipment-id={cardDefinition(card.kind).equipmentSlot ? card.id : undefined} data-judgement-id={!cardDefinition(card.kind).equipmentSlot ? card.id : undefined} style={{ visibility: hiddenCardIds.has(card.id) ? "hidden" : "visible" }}>
