...
export default function Home() {
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra: Record<string, unknown> = {}) {
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const responseCardAllowed = (item: Card) => duelResponse ? isAttackCard(item) : item.kind === "Dodge";
  ...
  useEffect(() => { if (!canRespond) { automaticDamage.current = ""; return; } const requiredKind = room.pendingDuel ? "Attack" : "Dodge"; if (busy || room.myHand.some((item) => requiredKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge")) return; const key = room.pendingDuel ? `duel-${room.pendingDuel.actorId}-${room.pendingDuel.opponentId}` : `attack-${room.pendingAttack?.sourceId}-${room.pendingAttack?.targetId}`; if (automaticDamage.current === key) return; automaticDamage.current = key; onActionRef.current(room.pendingDuel ? "take_duel_damage" : "take_damage"); }, [canRespond, busy, room.myHand, room.pendingAttack, room.pendingDuel]);
