...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const responseType = room.phase === "response" && !room.pendingHarvest && !room.pendingTargetCard && room.isMyAction
    ? room.currentAction?.kind ?? null
    : null;
  const invalidResponseState = room.phase === "response" && room.isMyAction && !room.pendingHarvest && !room.pendingTargetCard && !responseType;
  ...
  const greenDragonResponse = false;
  const rockCleavingResponse = false;
  ...
  const frostSwordResponse = false;
    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {triggerResponse && !greenDragonResponse && !rockCleavingResponse && !frostSwordResponse && <>{triggerOptions.map((option) => option.selection ? <button key={option.effectId} className={`serpent-control ${responseProviderId === option.effectId ? "active" : ""}`} disabled={responseControlsDisabled} onClick={() => { const active = responseProviderId === option.effectId; setResponseProviderId(active ? "" : option.effectId); setSelected(""); setSerpentSelected([]); setFrostSelectedKeys([]); }}>{responseProviderId === option.effectId ? `Cancel ${option.label}` : option.label}</button> : <button key={option.effectId} className="primary" disabled={responseControlsDisabled} onClick={() => onAction("trigger", { providerId: option.effectId })}>{busy ? "Resolving…" : option.label}</button>)}{selectedTriggerOption?.selection?.type === "target_cards" && (() => { const target = room.players.find((player) => player.id === selectedTriggerOption.selection.targetId); const eligible = new Set(selectedTriggerOption.selection.eligibleKeys); const max = selectedTriggerOption.selection.max; const toggle = (key: string) => setFrostSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < max ? [...keys, key] : keys); return <div className="hidden-card-picker"><small>Select cards from {target?.name ?? "the target"}</small>{target?.handCount ? <section><small>Hand (hidden)</small><div>{Array.from({ length: target.handCount }, (_, index) => { const key = `hand:${index}`; return <button type="button" disabled={responseControlsDisabled || !eligible.has(key)} className={triggerSelectionKeys.includes(key) ? "selected" : ""} key={key} onClick={() => toggle(key)}>?</button>; })}</div></section> : null}{target?.equipmentCards.length ? <section><small>Equipment</small><div>{target.equipmentCards.filter((item) => eligible.has(item.id)).map((item) => <button type="button" disabled={responseControlsDisabled} className={triggerSelectionKeys.includes(item.id) ? "selected named" : "named"} key={item.id} onClick={() => toggle(item.id)}>{cardDefinition(item.kind).name}</button>)}</div></section> : null}</div>; })()}{(selectedTriggerOption?.selection?.type === "cards" || selectedTriggerOption?.selection?.type === "target_cards") && <button className="primary" disabled={responseControlsDisabled || !triggerSelectionComplete} onClick={() => onAction("trigger", { providerId: selectedTriggerOption.effectId, ...(triggerSelectionUsesTargetCards ? { cardKeys: triggerSelectionKeys } : triggerSelectedCardIds.length === 1 ? { cardId: triggerSelectedCardIds[0] } : { cardIds: triggerSelectedCardIds }) })}>{busy ? "Resolving…" : `Use ${selectedTriggerOption.label}`}</button>}<button className="end" disabled={responseControlsDisabled || !canUseAction(room.currentAction, "decline_trigger")} onClick={() => onAction("decline_trigger")}>{busy ? "Skipping…" : "Skip reaction"}</button></>}
