...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const seatCountdown = displayedEvent ? { playerId: displayedEventPlayer?.id ?? room.meId, key: displayedEvent.id, durationMs: displayedEvent.type === "card" || displayedEvent.type === "cards" ? UI_TIMING.playedCard : UI_TIMING.eventMessage, deadline: 0, label: "Next step" }
    : room.phase === "response" && room.actionPlayerId && responseDeadline > 0 ? { playerId: room.actionPlayerId, key: `response-${room.actionPlayerId}-${responseDeadline}`, durationMs: 0, deadline: responseDeadline, label: "Respond" }
