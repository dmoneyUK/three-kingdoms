...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const [responseProviderId, setResponseProviderId] = useState("");
  const optimisticallyPresentedCards = useRef(new Set<string>());
  const [targetCardIndex, setTargetCardIndex] = useState<number | null>(null); const [targetCardZone, setTargetCardZone] = useState<"hand" | "equipment" | "judgement" | "">(""); const [targetCardId, setTargetCardId] = useState(""); const [frostSelecting, setFrostSelecting] = useState(false); const [frostSelectedKeys, setFrostSelectedKeys] = useState<string[]>([]);
  ...
  const [revealedResponseAt, setRevealedResponseAt] = useState(0);
  const responseVisibleAt = room.responseCountdownVisibleAt ?? 0;
  useEffect(() => { const timer = setTimeout(() => setRevealedResponseAt(responseVisibleAt), Math.max(0, responseVisibleAt - Date.now())); return () => clearTimeout(timer); }, [responseVisibleAt]);
  const hideNegationActor = Boolean(room.pendingNegation && !room.isMyAction && revealedResponseAt !== responseVisibleAt);
  const card = room.myHand.find((item) => item.id === selected); const current = room.players.find((player) => player.seat === room.turnSeat); const actor = hideNegationActor ? undefined : room.players.find((player) => player.id === room.actionPlayerId); const me = room.players.find((player) => player.id === room.meId); const targetPlayer = room.players.find((player) => player.id === target); const pendingTargetPlayer = room.players.find((player) => player.id === room.pendingTargetCard?.targetId); const pickerTarget = pendingTargetPlayer ?? targetPlayer;
  ...
  const greenDragonResponse = Boolean(room.pendingGreenDragon && room.isMyAction);
  const rockCleavingResponse = Boolean(room.pendingRockCleaving && room.isMyAction);
  const frostSwordResponse = room.phase === "response" && Boolean(room.pendingFrostSword) && room.isMyAction;
  const triggerResponse = room.currentAction?.kind === "trigger" && room.isMyAction;
  const frostTarget = room.players.find((player) => player.id === room.pendingFrostSword?.targetId);
  const responseTimerActive = canRespond || frostSwordResponse || triggerResponse;
  ...
  // mode first so the player can choose (for example) Qingguo over a black
  ...
  const requiredResponseKind = room.currentAction?.requirement === "negate" ? "Negation" : room.currentAction?.requirement === "attack" ? "Attack" : room.currentAction?.requirement === "dodge" ? "Dodge" : negationResponse ? "Negation" : greenDragonResponse || duelResponse || room.pendingGroup?.requiredKind === "Attack" ? "Attack" : responseType ? "Dodge" : null;
  const triggerOptions = room.currentAction?.triggerOptions ?? [];
  const selectedTriggerOption = triggerOptions.find((option) => option.effectId === responseProviderId) ?? null;
  ...
  const triggerSelectionUsesTargetCards = triggerSelection?.type === "target_cards";
  ...
  const triggerSelectionKeys = triggerSelectionUsesTargetCards ? frostSelectedKeys : [];
  const triggerSelectionComplete = Boolean(triggerSelection && ((triggerSelectionUsesCards && triggerSelectedCardIds.length >= triggerSelection.min && triggerSelectedCardIds.length <= triggerSelection.max) || (triggerSelectionUsesTargetCards && triggerSelectionKeys.length >= triggerSelection.min && triggerSelectionKeys.length <= triggerSelection.max)));
  const responseCardAllowed = (item: Card) => selectedResponseProvider?.selection?.type === "cards" ? selectedResponseProvider.selection.eligibleCardIds.includes(item.id) : triggerCardOption?.selection?.type === "cards" && triggerCardOption.selection.eligibleCardIds.includes(item.id) || (!semanticResponseOptions.length && Boolean(requiredResponseKind) && (rockCleavingResponse || greenDragonResponse || (requiredResponseKind === "Negation" ? item.kind === "Negation" : requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge")));
  const responsePlayAction = triggerResponse && selectedTriggerOption && triggerSelectionUsesCards ? "trigger" as GameplayAction : greenDragonResponse && canUseAction(room.currentAction, "trigger") ? "trigger" as GameplayAction : genericResponse && selectedResponseProvider ? "respond" as GameplayAction : (["respond_negation", "respond_green_dragon", "respond_group", "respond_duel", "respond_dodge"] as GameplayAction[]).find((action) => canUseAction(room.currentAction, action)) ?? null;
  const responseDamageAction = canUseAction(room.currentAction, "decline_response") ? "decline_response" as GameplayAction : (["pass_negation", "pass_green_dragon", "pass_rock_cleaving", "take_group_damage", "take_duel_damage", "take_damage"] as GameplayAction[]).find((action) => canUseAction(room.currentAction, action)) ?? null;
  ...
  const canFormSerpentAttack = hasSerpentSpear && room.myHand.length >= 2 && room.isMyTurn && canPlay && (room.phase !== "play-struck" || me?.hero === "zhang-fei");
  const responseDeadline = room.currentAction?.deadline ?? room.pendingNegation?.deadline ?? room.pendingGreenDragon?.deadline ?? room.pendingRockCleaving?.deadline ?? room.pendingFrostSword?.deadline ?? room.pendingGroup?.deadline ?? room.pendingDuel?.deadline ?? room.pendingAttack?.deadline ?? 0;
  ...
  const responseDecisionReady = (canRespond || frostSwordResponse || triggerResponse) && responsePresentationReady;
  ...
  useEffect(() => {
    const events = room.timeline.map((event) => ({
      id: event.id,
      drawPlayerId: event.type === "message" ? event.drawPlayerId : undefined,
    ...
    if (result.switched) {
      ...
      setInfoCard(null); setFrostSelecting(false); setFrostSelectedKeys([]);
      return;
    }
  ...
  useEffect(() => { if (!responseTimerActive || !responseDamageAction && !frostSwordResponse && !triggerResponse) { automaticResponseTimeout.current = ""; return; } if (busy || !responseDecisionReady) return; const key = room.actionRevision ?? `${room.actionPlayerId}-${room.currentAction?.kind ?? "response"}`; const startKey = `start-${key}`; const timerPrefix = `timer-${key}-`; if (automaticResponseTimeout.current !== startKey && !automaticResponseTimeout.current.startsWith(timerPrefix)) { automaticResponseTimeout.current = startKey; onActionRef.current("start_response_timer"); return; } if (responseDeadline <= 0) return; automaticResponseTimeout.current = `timer-${key}-${responseDeadline}`; const timer = setTimeout(() => { automaticResponseTimeout.current = `expired-${key}-${responseDeadline}`; const action = triggerResponse ? "decline_trigger" : frostSwordResponse ? "decline_trigger" : responseDamageAction; if (action) void onActionRef.current(action); }, Math.max(0, responseDeadline - Date.now())); return () => clearTimeout(timer); }, [responseTimerActive, frostSwordResponse, triggerResponse, busy, responseDecisionReady, responseDeadline, responseDamageAction, room.actionPlayerId, room.actionRevision, room.currentAction?.kind]);
  ...
  const playRockCleaving = async () => {
    if (!me || serpentSelected.length !== 2) return;
    const available = [...room.myHand, ...me.equipmentCards];
    const materials = serpentSelected.map((id) => available.find((item) => item.id === id)).filter((item): item is Card => Boolean(item));
    if (materials.length !== 2) return;
    const targetName = room.players.find((player) => player.id === room.pendingRockCleaving?.targetId)?.name ?? me.name;
    const optimisticEvent: CardGroupEvent = { id: `optimistic-rock-cleaving-${materials.map((item) => item.id).join("-")}`, type: "cards", player: me.name, target: targetName, cards: materials, action: "play", message: `${me.name} discards 2 cards with Rock Cleaving Axe to force the blocked Attack's damage on ${targetName}.` };
    const presentImmediately = !optimisticPlay && !activeEvent && eventQueue.length === 0;
    if (presentImmediately) {
      resolutionRevision.current += 1; materials.forEach((item) => optimisticallyPresentedCards.current.add(item.id));
      setResolutionClosing(false); setResolutionEvents((events) => appendUniqueEvents(events, [optimisticEvent])); setOptimisticPlay(optimisticEvent);
    }
    setSerpentSelected([]); setSelected("");
    const accepted = await onAction("trigger", { providerId: "rock_cleaving_axe_attack_dodged", cardIds: materials.map((item) => item.id) });
    if (!accepted && presentImmediately) {
      materials.forEach((item) => optimisticallyPresentedCards.current.delete(item.id));
      setOptimisticPlay(null); setResolutionEvents((events) => events.filter((event) => event.id !== optimisticEvent.id));
    }
  ...
  const commandPrompt = !responseDecisionReady && (canRespond || frostSwordResponse || triggerResponse) ? "Showing the current game event…"
    ...
    : canRespond ? rockCleavingResponse ? `Your action · choose 2 cards for Rock Cleaving Axe (${serpentSelected.length}/2), or skip` : responseProviderId && selectedResponseProvider && responseSelectionUsesCards ? `Your action · choose ${responseSelection.min === responseSelection.max ? responseSelection.min : `${responseSelection.min}-${responseSelection.max}`} card${responseSelection.max === 1 ? "" : "s"} for ${selectedResponseProvider.label} (${responseSelectedCardIds.length}/${responseSelection.max})` : requiredResponseKind === "Negation" ? `Your action · choose how to Negate ${room.pendingNegation?.responseTarget ?? room.pendingNegation?.cardName ?? "the latest effect"}, or skip` : greenDragonResponse ? "Your action · Green Dragon Blade may play another Attack on the same target, or skip" : `Your action · choose how to provide ${requiredResponseKind} for ${room.pendingGroup ? room.pendingGroup.cardKind === "SkyPiercingHalberdAttack" ? "Sky Piercing Halberd Attack" : cardDefinition(room.pendingGroup.cardKind).name : room.pendingDuel ? "the Duel" : "the Attack"}, or skip and take damage`
    : room.phase === "response" ? room.pendingNegation ? `Waiting for ${actor?.name ?? "the next player"} to answer ${room.pendingNegation.responseTarget ?? room.pendingNegation.cardName}` : room.pendingGreenDragon ? `Waiting for ${actor?.name ?? "the attacker"} to decide whether Green Dragon Blade continues` : room.pendingRockCleaving ? `Waiting for ${actor?.name ?? "the attacker"} to decide whether Rock Cleaving Axe forces damage` : room.pendingGroup ? `Waiting for ${actor?.name ?? "the target"} to play ${room.pendingGroup.requiredKind}` : room.pendingDuel ? `Waiting for ${actor?.name ?? "the duelist"} to play Attack` : `Waiting for ${defender?.name ?? "the target"} to answer ${attacker?.name ?? "the attacker"}`
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {rockCleavingResponse && <div className="weapon-response-backdrop"><section className="weapon-response-prompt" role="dialog" aria-label="Rock Cleaving Axe decision" aria-live="assertive"><span>ROCK CLEAVING AXE · YOUR RESPONSE</span><b>Your Attack was blocked by Dodge</b><small>Select exactly 2 cards from your hand or Equipment Zone, then force 1 damage — or skip the effect.</small><div><button className="primary" disabled={responseControlsDisabled || serpentSelected.length !== 2} onClick={playRockCleaving}>{busy ? "Discarding…" : "Use Rock Cleaving Axe"}</button><button className="end" disabled={responseControlsDisabled} onClick={() => onAction("decline_trigger")}>{busy ? "Skipping…" : "Skip Axe"}</button></div></section></div>}
      {frostSwordResponse && <div className={`weapon-response-backdrop ${frostSelecting ? "frost-selecting-backdrop" : ""}`}><section className="weapon-response-prompt" role="dialog" aria-label="Frost Sword decision" aria-live="assertive"><span>FROST SWORD · YOUR RESPONSE · {frostTarget?.name ?? "TARGET"}</span><b>{frostSelecting ? `Choose ${Math.min(2, frostTarget?.handCount ?? 0) || 1} target card${(frostTarget?.handCount ?? 0) > 1 ? "s" : ""}` : "Choose the Attack result"}</b><small>{frostSelecting ? `Select up to 2 cards from ${frostTarget?.name ?? "the target"}. You choose the cards; the target does not.` : `Prevent the damage and discard up to 2 cards from ${frostTarget?.name ?? "the target"}, or deal 1 damage.`}</small>{frostSelecting && frostTarget && <div className="frost-inline-picker" aria-label={`Choose cards from ${frostTarget.name}`}><strong>{frostTarget.name}&apos;s cards · {frostSelectedKeys.length}/2 selected</strong>{frostTarget.handCount > 0 && <section><small>Hand (hidden)</small><div>{Array.from({ length: frostTarget.handCount }, (_, index) => { const key = `hand:${index}`; return <button type="button" disabled={responseControlsDisabled} className={frostSelectedKeys.includes(key) ? "selected" : ""} aria-pressed={frostSelectedKeys.includes(key)} key={key} onClick={() => setFrostSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < 2 ? [...keys, key] : keys)}>?</button>; })}</div></section>}{frostTarget.equipmentCards.length > 0 && <section><small>Equipment</small><div>{frostTarget.equipmentCards.map((item) => <button type="button" disabled={responseControlsDisabled} className={frostSelectedKeys.includes(item.id) ? "selected named" : "named"} aria-pressed={frostSelectedKeys.includes(item.id)} key={item.id} onClick={() => setFrostSelectedKeys((keys) => keys.includes(item.id) ? keys.filter((key) => key !== item.id) : keys.length < 2 ? [...keys, item.id] : keys)}>{cardDefinition(item.kind).name}</button>)}</div></section>}</div>}<div>{frostSelecting ? <><button className="primary" disabled={responseControlsDisabled || frostSelectedKeys.length < 1} onClick={() => onAction("trigger", { providerId: "frost_sword_damage_about_to_apply", cardKeys: frostSelectedKeys })}>{busy ? "Resolving…" : `Discard ${frostSelectedKeys.length} selected`}</button><button className="end" disabled={responseControlsDisabled} onClick={() => { setFrostSelecting(false); setFrostSelectedKeys([]); }}>Back</button></> : <><button className="primary" disabled={responseControlsDisabled} onClick={() => { setFrostSelecting(true); setFrostSelectedKeys([]); }}>{busy ? "Opening…" : "Choose cards to discard"}</button><button className="end" disabled={responseControlsDisabled} onClick={() => onAction("decline_trigger")}>{busy ? "Resolving…" : "Deal 1 damage"}</button></>}</div></section></div>}
    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {triggerResponse && !greenDragonResponse && !rockCleavingResponse && !frostSwordResponse && <>{triggerOptions.map((option) => option.selection ? <button key={option.effectId} className={`serpent-control ${responseProviderId === option.effectId ? "active" : ""}`} disabled={responseControlsDisabled} onClick={() => { const active = responseProviderId === option.effectId; setResponseProviderId(active ? "" : option.effectId); setSelected(""); setSerpentSelected([]); setFrostSelectedKeys([]); }}>{responseProviderId === option.effectId ? `Cancel ${option.label}` : option.label}</button> : <button key={option.effectId} className="primary" disabled={responseControlsDisabled} onClick={() => onAction("trigger", { providerId: option.effectId })}>{busy ? "Resolving…" : option.label}</button>)}{selectedTriggerOption?.selection?.type === "target_cards" && <div className="hidden-card-picker"><small>Select cards from {room.players.find((player) => player.id === selectedTriggerOption.selection?.targetId)?.name ?? "the target"}</small><div>{Array.from({ length: room.players.find((player) => player.id === selectedTriggerOption.selection?.targetId)?.handCount ?? 0 }, (_, index) => { const key = `hand:${index}`; return <button type="button" disabled={responseControlsDisabled} className={triggerSelectionKeys.includes(key) ? "selected" : ""} key={key} onClick={() => setFrostSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < (selectedTriggerOption.selection?.type === "target_cards" ? selectedTriggerOption.selection.max : 0) ? [...keys, key] : keys)}>?</button>; })}</div></div>}{(selectedTriggerOption?.selection?.type === "cards" || selectedTriggerOption?.selection?.type === "target_cards") && <button className="primary" disabled={responseControlsDisabled || !triggerSelectionComplete} onClick={() => onAction("trigger", { providerId: selectedTriggerOption.effectId, ...(triggerSelectionUsesTargetCards ? { cardKeys: triggerSelectionKeys } : triggerSelectedCardIds.length === 1 ? { cardId: triggerSelectedCardIds[0] } : { cardIds: triggerSelectedCardIds }) })}>{busy ? "Resolving…" : `Use ${selectedTriggerOption.label}`}</button>}<button className="end" disabled={responseControlsDisabled || !canUseAction(room.currentAction, "decline_trigger")} onClick={() => onAction("decline_trigger")}>{busy ? "Skipping…" : "Skip reaction"}</button></>}
          {(canRespond || greenDragonResponse) && <>{rockCleavingResponse ? <button className="primary" disabled={responseControlsDisabled || serpentSelected.length !== 2} onClick={playRockCleaving}>{busy ? "Discarding…" : "Use Rock Cleaving Axe"}</button> : semanticResponseOptions.length && genericResponse ? <>{semanticResponseOptions.filter((option) => option.activation === "explicit").map((option) => option.selection ? <button key={option.providerId} className={`serpent-control ${responseProviderId === option.providerId ? "active" : ""}`} disabled={responseControlsDisabled} onClick={() => { const active = responseProviderId === option.providerId; setResponseProviderId(active ? "" : option.providerId); setSelected(""); setSerpentSelected([]); }}>{responseProviderId === option.providerId ? `Cancel ${option.label}` : option.label}</button> : <button key={option.providerId} className="primary" disabled={responseControlsDisabled} onClick={() => submitResponseProvider(option)}>{busy ? "Resolving…" : option.label}</button>)}{selectedResponseProvider?.selection && <button className="primary" disabled={responseControlsDisabled || !responseSelectionComplete} onClick={() => submitResponseProvider()}>{busy ? "Playing…" : selectedResponseProvider.activation === "implicit" ? selectedResponseProvider.label : `Use ${selectedResponseProvider.label}`}</button>}</> : <button className="primary" disabled={responseControlsDisabled || !card || !responseCardAllowed(card)} onClick={playResponseCard}>{busy ? "Playing…" : greenDragonResponse ? "Continue Attack" : `Play ${requiredResponseKind}`}</button>}<button className="end" disabled={responseControlsDisabled || !(greenDragonResponse ? canUseAction(room.currentAction, "decline_trigger") : responseDamageAction)} onClick={() => greenDragonResponse ? onAction("decline_trigger") : responseDamageAction && onAction(responseDamageAction)}>{busy ? "Skipping…" : requiredResponseKind === "Negation" ? "Skip response" : greenDragonResponse ? "Skip follow-up" : rockCleavingResponse ? "Skip Axe" : "Skip · take 1 damage"}</button></>}
      ...
      <div className="play-hand">{room.myHand.map((item) => { const definition = cardDefinition(item.kind); const costSelection = serpentMode || rockCleavingResponse || canRespond && responseSelectionUsesCards && responseSelectionMax > 1 || triggerResponse && triggerSelectionUsesCards && triggerSelectionMax > 1; const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : costSelection ? serpentSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || responseDecisionReady || rescueDecisionReady; const responseSelectionLimit = triggerResponse && triggerSelectionUsesCards ? triggerSelection.max : responseSelectionUsesCards ? responseSelection.max : 2; return <div className="card-slot" key={item.id}><button disabled={!maySelect || (responseDecisionReady && (canRespond || greenDragonResponse || triggerResponse) && !responseCardAllowed(item)) || (rescueDecisionReady && item.kind !== "Peach")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else if (costSelection) setSerpentSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < responseSelectionLimit ? [...ids, item.id] : ids); else { setSelected((id) => id === item.id ? "" : item.id); setTarget(""); } setTargetCardIndex(null); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-name-mark">{definition.name}</span><strong>{definition.category} card</strong></button><button type="button" className="card-info-button" aria-label={`Explain ${definition.name}`} onClick={() => setInfoCard(item)}>i</button></div>; })}</div>
...
function TableResolutionSequence({ events, activeEvent, waitingReason, players, myTableIndex, concluding }: { events: GameEvent[]; activeEvent: GameEvent | null; waitingReason: string; players: Player[]; myTableIndex: number; concluding: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    if (!reveal || !table || !equipmentFlightId) return;
    const destination = Array.from(table.querySelectorAll<HTMLElement>("[data-equipment-id]"))
      .find((slot) => slot.dataset.equipmentId === equipmentFlightId);
    ...
    const measure = () => {
      ...
      reveal.style.setProperty("--equipment-x", `${slot.left + slot.width / 2 - bounds.left}px`);
      reveal.style.setProperty("--equipment-y", `${slot.top + slot.height / 2 - bounds.top}px`);
      reveal.style.setProperty("--equipment-scale-x", String(slot.width / face.offsetWidth));
      reveal.style.setProperty("--equipment-scale-y", String(slot.height / face.offsetHeight));
    };
    measure();
