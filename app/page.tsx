...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className="play-table">
      ...
      {activeEvent?.type === "message" && !roleReveal && <div className="game-event-stage" role="status"><div><Countdown key={activeEvent.id} durationMs={UI_TIMING.eventMessage} /><span>GAME EVENT</span><b>{activeEvent.message}</b></div></div>}
      {roleReveal && activeEvent && <div className="game-event-stage role-reveal-stage" role="status"><div><Countdown key={activeEvent.id} durationMs={UI_TIMING.eventMessage} /><span>ROLE REVEALED</span><b>{roleReveal[1]}</b><strong>{roleReveal[2]}</strong><small>This player’s hidden allegiance is now public.</small></div></div>}
      ...
      {shownCard && <div className="played-card-stage" role="status"><Countdown key={shownCard.id} durationMs={UI_TIMING.playedCard} /><div className="card-action-title"><b>{shownCard.player}</b><span>{cardEventVerb(shownCard, peachRescue)}</span>{cardEventHasTarget(shownCard, peachRescue) && <strong>{shownCard.target}</strong>}</div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="action-direction"><span>{cardEventDirection(shownCard, peachRescue)}</span><CardFace card={shownCard.card} /></div>{cardEventHasTarget(shownCard, peachRescue) && <ActionHeroCard player={actionTarget} label={shownCard.card.kind === "Dodge" ? "ATTACKER" : peachRescue ? "DYING PLAYER" : "TARGET"} />}</div></div>}
      {activeCards && <div className="played-card-stage discard-group-stage" role="status"><Countdown key={activeCards.id} durationMs={UI_TIMING.playedCard} /><div className="card-action-title"><b>{activeCards.player}</b><span>{activeCards.action === "reveal" ? "reveals for Bumper Harvest" : "discards together"}</span></div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="discard-card-row">{activeCards.cards.map((shown) => <CardFace card={shown} key={shown.id} />)}</div></div></div>}
      ...
      {room.players.map((player, index) => { const relativeIndex = (index - myTableIndex + room.players.length) % room.players.length; const angle = 180 + (360 / room.players.length) * relativeIndex; const targetable = Boolean(card && (isAttackCard(card) ? (player.distance ?? 99) <= 1 : card.kind === "Dismantle" ? player.handCount > 0 : card.kind === "Steal" ? player.handCount > 0 && (player.distance ?? 99) <= 1 : card.kind === "Duel" || card.kind === "Overindulgence" && !player.judgementCards.some((delayed) => delayed.kind === "Overindulgence"))); return <button disabled={!room.isMyTurn || !canPlay || !player.alive || player.id === room.meId || !targetable} onClick={() => { setTarget(player.id); setTargetCardIndex(null); }} className={`started-player play-seat ${player.id === room.meId ? "self-player" : ""} ${player.seat === room.turnSeat && room.status === "playing" ? "active-turn" : ""} ${player.id === room.actionPlayerId && room.status === "playing" ? "active-action" : ""} ${target === player.id ? "targeted" : ""} ${!player.alive ? "defeated" : ""}`} key={player.id} style={{ "--angle": `${angle}deg` } as React.CSSProperties}><div className="seal">{player.name[0]}</div><b>{player.name}</b><small>{heroName(player.hero)} · {player.role ?? "Role hidden"}</small><span>{"♥".repeat(player.hp ?? 0)} · {player.handCount} cards{player.id !== room.meId ? ` · distance ${player.distance}` : ""}</span>{player.judgementCards.length > 0 && <em className="judgement-zone">Judgement · {player.judgementCards.map((delayed) => cardDefinition(delayed.kind).name).join(", ")}</em>}</button>; })}
