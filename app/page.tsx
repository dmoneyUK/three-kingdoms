...
export default function Home() {
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage", extra: Record<string, string> = {}) {
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage", extra?: Record<string, string>) => void; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b><span>{current?.name ?? "—"}&apos;s turn</span></div><button className="text-button" onClick={onLeave}>Exit</button></header>
    ...
    <footer className="play-command"><div className="turn-controls"><span>{room.status === "finished" ? "The match has ended" : room.phase === "response" ? room.pendingAttack?.targetId === room.meId ? "You are under attack — play Dodge or take damage" : "Waiting for the defender to answer Strike" : room.isMyTurn ? room.phase === "draw" ? "Your draw phase" : card?.kind === "Strike" && !target ? "Choose an adjacent opponent (distance 1)" : room.phase === "play-struck" && me?.hero !== "zhang-fei" ? "Strike used — play Peach or end your turn" : "Your play phase" : `Waiting for ${current?.name ?? "another player"}`}</span><div>{room.phase === "response" && room.pendingAttack?.targetId === room.meId && <><button className="primary" disabled={busy || !room.myHand.some((item) => item.kind === "Dodge")} onClick={() => onAction("respond_dodge")}>Play Dodge</button><button className="end" disabled={busy} onClick={() => onAction("take_damage")}>Take damage</button></>}{room.isMyTurn && room.phase === "draw" && <button className="primary" disabled={busy} onClick={() => onAction("draw")}>Draw 2 cards</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || !card || (card.kind === "Strike" && (!target || (room.phase === "play-struck" && me?.hero !== "zhang-fei"))) || card.kind === "Dodge"} onClick={play}>Play selected</button><button className="end" disabled={busy} onClick={() => onAction("end_turn")}>End turn</button></>}</div></div>
