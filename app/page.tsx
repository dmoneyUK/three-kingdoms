...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const [selected, setSelected] = useState(""); const [wushengMode, setWushengMode] = useState<"play" | "response" | null>(null); const [targetIds, setTargetIds] = useState<string[]>([]); const target = targetIds[0] ?? "";
  ...
  const triggerSelectionComplete = Boolean(triggerSelection && ((triggerSelectionUsesCards && triggerSelectedCardIds.length >= triggerSelection.min && triggerSelectedCardIds.length <= triggerSelection.max) || (triggerSelectionUsesChoice && triggerChoice === "draw" || triggerSelectionUsesChoice && triggerChoice === "discard" && triggerSelectionKeys.length === 1)));
  ...
  const selectedCanPlayAsAttack = Boolean(card && (isAttackCard(card) || wushengMode === "play" && playPhaseAttackCardIds.has(card.id)));
  ...
  useEffect(() => { const timer = setTimeout(() => { setSelected(""); setWushengMode(null); setTargetIds([]); setTargetCardIndex(null); setTargetCardZone(""); setTargetCardId(""); setDiscardSelected([]); setSerpentMode(false); setSerpentSelected([]); setResponseProviderId(""); }, 0); return () => clearTimeout(timer); }, [room.turnSeat, room.phase, room.meId]);
  ...
  useEffect(() => { const timer = setTimeout(() => { setWushengMode(null); setResponseProviderId(""); setSelected(""); setSerpentSelected([]); setTriggerSelectedKeys([]); setTriggerChoice(""); }, 0); return () => clearTimeout(timer); }, [room.actionRevision]);
  useEffect(() => {
...
function TargetCardPicker({ option, selection, target, selectedKeys, disabled, busy, canDecline, error, isQuickTest, buildRevision, onToggle, onUse, onDecline }: { option: TriggerOptionView; selection: TargetCardSelection; target: Player; selectedKeys: string[]; disabled: boolean; busy: boolean; canDecline: boolean; error: string; isQuickTest: boolean; buildRevision?: string; onToggle: (key: string) => void; onUse: (keys: string[]) => void; onDecline: () => void }) {
  ...
  const debugText = isQuickTest
    ? `keys=${JSON.stringify(selection.eligibleKeys)} handCount=${target.handCount} equipmentIds=${JSON.stringify(target.equipmentCards.map((item) => item.id))} buildRevision=${buildRevision ?? "unknown"}`
    : null;
  ...
  return <div className="target-card-picker-overlay" role="presentation"><section className="target-card-picker-panel" role="dialog" aria-modal="true" aria-label={`${effectLabel} target card selection`}>
    ...
    {debugText && <small className="target-card-picker-debug">{debugText}</small>}
    <div className="target-card-picker-card-row" aria-label="Eligible cards">
      {items.map((item) => <button type="button" key={item.key} className={`target-card-picker-card ${item.hidden ? "hidden" : "equipment"} ${validSelectedKeys.includes(item.key) ? "selected" : ""}`} disabled={disabled} aria-pressed={validSelectedKeys.includes(item.key)} aria-label={item.label} onClick={() => onToggle(item.key)}>{item.hidden ? <span aria-hidden="true">?</span> : item.card && <CardFace card={item.card} />}{validSelectedKeys.includes(item.key) && <span className="target-card-picker-check" aria-hidden="true">✓</span>}</button>)}
    </div>
    ...
    <div className="target-card-picker-actions">
      {canDecline && <button type="button" className="end" disabled={disabled} onClick={onDecline}>{busy ? "Skipping…" : "Skip reaction"}</button>}
      <button type="button" className="primary" disabled={disabled || !complete} onClick={() => onUse(validSelectedKeys)}>{busy ? "Resolving…" : `Use ${effectLabel}`}</button>
    </div>
    {error && <p className="error" role="alert">{error}</p>}
  </section></div>;
}
