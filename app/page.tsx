...
export default function Home() {
  ...
  const [drawNotice, setDrawNotice] = useState(0);
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "give_peach" | "skip_rescue", extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      if (action === "draw" && data.drawnCards?.length) setDrawNotice(data.drawnCards.length);
  ...
  function leave() {
    localStorage.removeItem("three-realms-session"); setRoom(null); setToken(""); setCode(""); setError(""); setDrawNotice(0);
  }

  if (room?.status === "started" || room?.status === "playing" || room?.status === "finished") return <GameRoom room={room} busy={busy} error={error} drawNotice={drawNotice} onDrawNoticeEnd={() => setDrawNotice(0)} onAction={send} onLeave={leave} />;
...
function GameRoom({ room, busy, error, drawNotice, onDrawNoticeEnd, onAction, onLeave }: { room: Room; busy: boolean; error: string; drawNotice: number; onDrawNoticeEnd: () => void; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const [turnNotice, setTurnNotice] = useState(""); const onActionRef = useRef(onAction); const onDrawNoticeEndRef = useRef(onDrawNoticeEnd);
  ...
  useEffect(() => { onDrawNoticeEndRef.current = onDrawNoticeEnd; }, [onDrawNoticeEnd]);
  ...
  useEffect(() => { if (drawNotice || activeEvent || !eventQueue.length) return; const timer = setTimeout(() => { setActiveEvent(eventQueue[0]); setEventQueue((queue) => queue.slice(1)); }, 0); return () => clearTimeout(timer); }, [drawNotice, activeEvent, eventQueue]);
  useEffect(() => { if (!activeEvent) return; const timer = setTimeout(() => setActiveEvent(null), 5000); return () => clearTimeout(timer); }, [activeEvent]);
  useEffect(() => { if (!drawNotice) return; const timer = setTimeout(() => onDrawNoticeEndRef.current(), 5000); return () => clearTimeout(timer); }, [drawNotice]);
  ...
  useEffect(() => { if (!canRescue) { automaticRescueSkip.current = ""; return; } if (busy || presentationBusy) return; const key = `${room.pendingDying?.targetId}-${room.actionPlayerId}`; if (automaticRescueSkip.current === key) return; const timer = setTimeout(() => { automaticRescueSkip.current = key; onActionRef.current("skip_rescue"); }, 5000); return () => clearTimeout(timer); }, [canRescue, busy, presentationBusy, room.pendingDying?.targetId, room.actionPlayerId]);
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className="play-table">
      ...
      {drawNotice > 0 && <div className="draw-reveal-stage" role="status"><div className="draw-reveal-title"><span>TURN STARTED</span><b>{current?.name ?? "Player"} is drawing {drawNotice} cards</b><small>Draw Phase</small></div></div>}
      ...
      {activeCard && <div className="played-card-stage" role="status"><div className="card-action-title"><b>{activeCard.player}</b><span>{activeCard.action === "discard" ? "discards" : activeCard.card.kind === "Dodge" ? "blocks" : peachRescue ? "rescues" : activeCard.card.kind === "Peach" ? "heals self" : "attacks"}</span>{activeCard.action !== "discard" && (activeCard.card.kind !== "Peach" || peachRescue) && <strong>{activeCard.target}</strong>}</div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="action-direction"><span>{activeCard.action === "discard" ? "DISCARD ↓" : peachRescue ? "GIVES →" : activeCard.card.kind === "Peach" ? "SELF" : activeCard.card.kind === "Dodge" ? "← BLOCKS" : "ATTACKS →"}</span><div className={`played-card ${activeCard.card.kind.toLowerCase()}`}><i>{activeCard.card.rank}<small>{activeCard.card.suit}</small></i><b>{activeCard.card.kind === "Strike" ? "⚔" : activeCard.card.kind === "Dodge" ? "盾" : "桃"}</b><strong>{activeCard.card.kind}</strong></div></div>{activeCard.action !== "discard" && (activeCard.card.kind !== "Peach" || peachRescue) && <ActionHeroCard player={actionTarget} label={activeCard.card.kind === "Dodge" ? "ATTACKER" : peachRescue ? "DYING PLAYER" : "TARGET"} />}</div></div>}
      {activeCards && <div className="played-card-stage discard-group-stage" role="status"><div className="card-action-title"><b>{activeCards.player}</b><span>discards together</span></div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="discard-card-row">{activeCards.cards.map((discarded) => <div className={`played-card ${discarded.kind.toLowerCase()}`} key={discarded.id}><i>{discarded.rank}<small>{discarded.suit}</small></i><b>{discarded.kind === "Strike" ? "⚔" : discarded.kind === "Dodge" ? "盾" : "桃"}</b><strong>{discarded.kind}</strong></div>)}</div></div></div>}
    ...
    <footer className="play-command"><div className="turn-controls"><span>{presentationBusy ? "Showing the current game event…" : room.status === "finished" ? "The match has ended" : room.phase === "dying" ? room.isMyAction ? `Your action · give Peach to ${dyingPlayer?.name ?? "the dying player"} or skip · auto-skip in 5s` : `Waiting for ${actor?.name ?? "a player"} to decide whether to give Peach to ${dyingPlayer?.name ?? "the dying player"}` : canRespond ? `Your action · select a Dodge card, then play it—or take damage` : room.phase === "response" ? `Waiting for ${defender?.name ?? "the target"} to answer ${attacker?.name ?? "the attacker"}` : room.phase === "discard" && room.isMyTurn ? `Your action · Discard Phase · select ${excessCards} card${excessCards === 1 ? "" : "s"} (${discardSelected.length}/${excessCards})` : room.isMyTurn ? room.phase === "draw" ? "Your action · Draw Phase" : card?.kind === "Strike" && !target ? "Your action · Play Phase · choose an adjacent target" : room.phase === "play-struck" ? "Your action · Play Phase · Strike used" : "Your action · Play Phase" : `Waiting for ${actor?.name ?? current?.name ?? "another player"}`}</span><div>{canRescue && <><button className="primary" disabled={busy || presentationBusy || !room.myHand.some((item) => item.kind === "Peach")} onClick={() => onAction("give_peach")}>Give Peach</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("skip_rescue")}>Skip</button></>}{canRespond && <><button className="primary" disabled={busy || card?.kind !== "Dodge"} onClick={() => card && onAction("respond_dodge", { cardId: card.id })}>Play selected</button><button className="end" disabled={busy} onClick={() => onAction("take_damage")}>Take 1 damage</button></>}{room.isMyTurn && room.phase === "discard" && <button className="end" disabled={busy || discardSelected.length !== excessCards} onClick={() => onAction("discard_cards", { cardIds: discardSelected })}>Discard {excessCards} selected</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || drawNotice > 0 || !card || (card.kind === "Strike" && (!target || room.phase === "play-struck")) || card.kind === "Dodge"} onClick={play}>Play selected</button><button className="end" disabled={busy || drawNotice > 0} onClick={() => onAction("end_turn")}>Finish Play Phase</button></>}</div></div>
      <div className="play-hand">{room.myHand.map((item) => { const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || canRespond; return <button key={item.id} disabled={!maySelect || (canRespond && item.kind !== "Dodge")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else setSelected((id) => id === item.id ? "" : item.id); setTarget(""); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-glyph">{item.kind === "Strike" ? "⚔" : item.kind === "Dodge" ? "盾" : "桃"}</span><strong>{item.kind}</strong><small>{room.phase === "discard" ? isSelected ? "Selected · tap to remove" : "Tap to select" : canRespond && item.kind === "Dodge" ? isSelected ? "Selected · play it" : "Tap to answer Strike" : item.kind === "Dodge" ? "Play only when targeted" : item.kind === "Peach" ? "Recover 1 HP" : "Range 1 · deal 1 damage"}</small></button>; })}</div>
...
function describeEvent(event: GameEvent) {
  ...
  if (event.type === "cards") return `${event.player} reveals and discards ${event.cards.map((card) => `${card.rank}${card.suit} ${card.kind}`).join(", ")}.`;
