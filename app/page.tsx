    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          <div className="play-hand">{room.myHand.map((item) => { const definition = cardDefinition(item.kind); const costSelection = serpentMode || canRespond && responseSelectionUsesCards && responseSelectionMax > 1 || triggerResponse && triggerSelectionUsesCards && triggerSelectionMax > 1; const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : activeSkillSelection ? activeSkillSelectedCardIds.includes(item.id) : costSelection ? serpentSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || responseDecisionReady || rescueDecisionReady; const responseSelectionLimit = triggerResponse && triggerSelectionUsesCards ? triggerSelection.max : responseSelectionUsesCards ? responseSelection.max : 2; const skillModeCardDisabled = Boolean(activeSkillTargetSelection || wushengMode && !wushengEligibleCardIds.has(item.id) || longdanMode && !longdanEligibleCardIds.has(item.id) || activeSkillSelection && !activeSkillSelection.eligibleCardIds.includes(item.id)); const skillModeEligible = wushengMode && wushengEligibleCardIds.has(item.id) || longdanMode && longdanEligibleCardIds.has(item.id) || activeSkillSelection?.eligibleCardIds.includes(item.id) === true; return <div className="card-slot" key={item.id}><button disabled={!maySelect || skillModeCardDisabled || (responseDecisionReady && (canRespond || triggerResponse) && !responseCardAllowed(item)) || (rescueDecisionReady && item.kind !== "Peach")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else if (activeSkillSelection) setActiveSkillSelectionState((state) => { if (!state || !activeSkillStateIsCurrent) return state; const validIds = state.cardIds.filter((id) => activeSkillSelection.eligibleCardIds.includes(id)); return validIds.includes(item.id) ? { ...state, cardIds: validIds.filter((id) => id !== item.id) } : validIds.length < activeSkillSelection.max ? { ...state, cardIds: [...validIds, item.id] } : { ...state, cardIds: validIds }; }); else if (costSelection) setSerpentSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < responseSelectionLimit ? [...ids, item.id] : ids); else { setSelected((id) => id === item.id ? "" : item.id); setTarget(""); } setTargetCardIndex(null); }} className={`game-card ${item.kind.toLowerCase()} ${suitColorClass(item.suit)} ${isSelected ? "selected" : ""} ${skillModeEligible ? "hero-skill-eligible" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-name-mark">{definition.name}</span><strong>{definition.category} card</strong></button><button type="button" className="card-info-button" aria-label={`Explain ${definition.name}`} onClick={() => setInfoCard(item)}>i</button></div>; })}</div>
      {activeSkillSelection && me?.equipmentCards.length ? <div className="play-hand active-skill-equipment" aria-label="Equipped cards eligible for this skill">{me.equipmentCards.map((equipment) => activeSkillSelection.eligibleCardIds.includes(equipment.id) ? <button type="button" key={equipment.id} className={`game-card ${equipment.kind.toLowerCase()} ${activeSkillSelectedCardIds.includes(equipment.id) ? "selected" : ""}`} disabled={busy || presentationBusy} onClick={() => setActiveSkillSelectionState((state) => { if (!state || !activeSkillStateIsCurrent) return state; const validIds = state.cardIds.filter((id) => activeSkillSelection.eligibleCardIds.includes(id)); return validIds.includes(equipment.id) ? { ...state, cardIds: validIds.filter((id) => id !== equipment.id) } : validIds.length < activeSkillSelection.max ? { ...state, cardIds: [...validIds, equipment.id] } : { ...state, cardIds: validIds }; })}><span className="corner">{equipment.rank}<i>{equipment.suit}</i></span><span className="card-name-mark">{cardDefinition(equipment.kind).name}</span><strong>equipment card</strong></button> : null)}</div> : null}{error && <p className="error play-error" role="alert">{error}</p>}<div className="self-summary"><strong>{room.myRole}</strong> · {heroName(me?.hero)} · {me?.hp}/{me?.maxHp} HP · Your position is always at the bottom</div>
    </footer>
