...
function pendingTimelineSequence(room: Room) {
  const sourceId = room.pendingTargetCard?.sourceId ?? room.pendingNegation?.sourceId ?? room.pendingDuel?.sourceId ?? room.pendingAttack?.sourceId ?? room.pendingGreenDragon?.sourceId ?? room.pendingRockCleaving?.sourceId ?? room.pendingGroup?.sourceId;
  const source = room.players.find((player) => player.id === sourceId);
  if (!source) return [];
  const expectedName = room.pendingTargetCard ? cardDefinition(room.pendingTargetCard.cardKind).name : room.pendingNegation?.cardName ?? (room.pendingDuel ? "Duel" : room.pendingGroup ? cardDefinition(room.pendingGroup.cardKind).name : room.pendingAttack || room.pendingGreenDragon || room.pendingRockCleaving ? "Attack" : null);
...
export default function Home() {
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "serpent_spear_attack" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_rock_cleaving" | "pass_rock_cleaving" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "choose_target_card" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra: Record<string, unknown> = {}) {
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "serpent_spear_attack" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_rock_cleaving" | "pass_rock_cleaving" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "choose_target_card" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const [targetCardIndex, setTargetCardIndex] = useState<number | null>(null); const [targetCardZone, setTargetCardZone] = useState<"hand" | "equipment" | "judgement" | "">(""); const [targetCardId, setTargetCardId] = useState("");
  ...
  const canRespond = room.phase === "response" && !room.pendingHarvest && !room.pendingTargetCard && room.isMyAction;
  ...
  const responseDeadline = room.pendingNegation?.deadline ?? room.pendingGreenDragon?.deadline ?? room.pendingRockCleaving?.deadline ?? room.pendingGroup?.deadline ?? room.pendingDuel?.deadline ?? room.pendingAttack?.deadline ?? 0;
  ...
  const playSerpentAttack = async () => {
    ...
    if (!action) return;
    setSerpentMode(false); setSerpentSelected([]); setSelected(""); setTarget("");
