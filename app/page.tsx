...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => fetchRoom(roomCode, token, true), room?.pendingHarvest ? UI_TIMING.harvestPoll : UI_TIMING.roomPoll);
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  useEffect(() => { const fresh = (room.timeline ?? []).filter((event) => !seenEvents.current.has(event.id)); fresh.forEach((event) => seenEvents.current.add(event.id)); const visible = fresh.filter((event) => { if (event.presentation === false) return false; if (event.type === "card" && optimisticallyPresentedCards.current.delete(event.card.id)) return false; return true; }); if (visible.length) { if (!activeEvent && eventQueue.length === 0) { setActiveEvent(visible[0]); setEventQueue(visible.slice(1)); } else setEventQueue((queue) => [...queue, ...visible]); } setProcessedTimelineKey(timelineKey); }, [room.timeline, timelineKey, activeEvent, eventQueue.length]);
