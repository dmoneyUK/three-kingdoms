...
type PresentationEventMeta = { resolutionId?: string; importance?: PresentationImportance; finalResult?: boolean; playedAs?: "attack" };
...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const equipmentInFlight = new Set(pendingPresentationEvents
    .flatMap((event) => event.type === "card" && event.action === "equip" && !event.playedAs ? [event.card.id] : []));
