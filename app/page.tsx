...
type CardEvent = { id: string; player: string; target: string; card: Card };
...
type Room = { code: string; status: "lobby" | "heroes" | "started" | "playing" | "finished"; maxPlayers: number; isHost: boolean; meId: string; myRole: string | null; myHeroOptions: Hero[]; players: Player[]; myHand: Card[]; turnSeat: number | null; phase: string | null; deckCount: number; discardTop: Card | null; log: string[]; cardEvents: CardEvent[]; isMyTurn: boolean; pendingAttack: { sourceId: string; targetId: string } | null };
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_card", extra?: Record<string, string>) => void; onLeave: () => void }) {
  ...
  const [cardQueue, setCardQueue] = useState<CardEvent[]>([]); const [activeCard, setActiveCard] = useState<CardEvent | null>(null); const seenCards = useRef(new Set((room.cardEvents ?? []).map((event) => event.id)));
  ...
  useEffect(() => { const unseenCards = (room.cardEvents ?? []).some((event) => !seenCards.current.has(event.id)); if (room.status !== "playing" || activeCard || cardQueue.length || unseenCards || room.turnSeat === previousTurn.current) return; previousTurn.current = room.turnSeat; setTurnNotice(`${current?.name ?? "Player"}'s turn`); const timer = setTimeout(() => setTurnNotice(""), 1600); return () => clearTimeout(timer); }, [room.turnSeat, room.status, current?.name, room.cardEvents, activeCard, cardQueue.length]);
  useEffect(() => { const fresh = (room.cardEvents ?? []).filter((event) => !seenCards.current.has(event.id)); fresh.forEach((event) => seenCards.current.add(event.id)); if (fresh.length) setCardQueue((queue) => [...queue, ...fresh]); }, [room.cardEvents]);
  useEffect(() => { if (activeCard || !cardQueue.length) return; setActiveCard(cardQueue[0]); setCardQueue((queue) => queue.slice(1)); }, [activeCard, cardQueue]);
  useEffect(() => { if (!activeCard) return; const timer = setTimeout(() => setActiveCard(null), 5000); return () => clearTimeout(timer); }, [activeCard]);
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b><span>{current?.name ?? "—"}&apos;s turn</span></div><button className="text-button" onClick={onLeave}>Exit</button></header>
    <section className="play-table"><div className="role-reveal"><span>YOUR SECRET ROLE</span><b>{room.myRole}</b><small>{room.myRole === "Lord" ? "Survive and eliminate every Rebel and Renegade." : room.myRole === "Loyalist" ? "Protect the Lord and eliminate every threat." : room.myRole === "Rebel" ? "Overthrow the Lord." : "Be the last player standing."}</small></div>
      ...
      {activeCard && <div className="played-card-stage" role="status"><div className="card-action-title"><b>{activeCard.player}</b><span>{activeCard.card.kind === "Dodge" ? "blocks" : activeCard.card.kind === "Peach" ? "heals self" : "attacks"}</span>{activeCard.card.kind !== "Peach" && <strong>{activeCard.target}</strong>}</div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="action-direction"><span>{activeCard.card.kind === "Peach" ? "SELF" : activeCard.card.kind === "Dodge" ? "← BLOCKS" : "ATTACKS →"}</span><div className={`played-card ${activeCard.card.kind.toLowerCase()}`}><i>{activeCard.card.rank}<small>{activeCard.card.suit}</small></i><b>{activeCard.card.kind === "Strike" ? "⚔" : activeCard.card.kind === "Dodge" ? "盾" : "桃"}</b><strong>{activeCard.card.kind}</strong></div></div>{activeCard.card.kind !== "Peach" && <ActionHeroCard player={actionTarget} label={activeCard.card.kind === "Dodge" ? "ATTACKER" : "TARGET"} />}</div></div>}
