...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const play = () => { if (!card) return; const needsTarget = isAttackCard(card) || card.kind === "Dismantle" || card.kind === "Steal"; onAction("play_card", { cardId: card.id, ...(needsTarget ? { targetId: target } : {}), ...(["Dismantle", "Steal"].includes(card.kind) ? { targetCardIndex } : {}) }); setSelected(""); setTarget(""); setTargetCardIndex(null); };
...
function cardEventHasTarget(event: CardEvent, peachRescue: boolean) {
  return event.action !== "discard" && (isAttackCard(event.card) || event.card.kind === "Dodge" || event.card.kind === "Dismantle" || event.card.kind === "Steal" || peachRescue);
}
