...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const cardResponseOption = semanticResponseOptions.find((option) => option.providerId === "card");
  const serpentResponseOption = semanticResponseOptions.find((option) => option.providerId === "serpent_spear_attack");
  const eightTrigramsResponseOption = semanticResponseOptions.find((option) => option.providerId === "eight_trigrams_dodge");
  ...
  const responseCardAllowed = (item: Card) => cardResponseOption?.selection?.type === "cards" ? cardResponseOption.selection.eligibleCardIds.includes(item.id) : Boolean(requiredResponseKind) && (rockCleavingResponse || (requiredResponseKind === "Negation" ? item.kind === "Negation" : requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge"));
  const canUseEightTrigrams = Boolean(eightTrigramsResponseOption) || canUseAction(room.currentAction, "respond_eight_trigrams");
  const responsePlayAction = genericResponse && cardResponseOption ? "respond" as GameplayAction : (["respond_negation", "respond_green_dragon", "respond_group", "respond_duel", "respond_dodge"] as GameplayAction[]).find((action) => canUseAction(room.currentAction, action)) ?? null;
  ...
  const canFormSerpentAttack = hasSerpentSpear && room.myHand.length >= 2 && (canRespond && requiredResponseKind === "Attack" && (Boolean(serpentResponseOption) || canUseAction(room.currentAction, groupResponse ? "respond_group" : "respond_duel")) && !greenDragonResponse || room.isMyTurn && canPlay && (room.phase !== "play-struck" || me?.hero === "zhang-fei"));
  ...
  const playSerpentAttack = async () => {
    ...
    const action = canPlay ? "serpent_spear_attack" : genericResponse && serpentResponseOption ? "respond" : groupResponse ? "respond_group" : duelResponse ? "respond_duel" : null;
    if (!action) return;
    setSerpentMode(false); setSerpentSelected([]); setSelected(""); setTargetIds([]);
    const accepted = await onAction(action, { cardIds: materials.map((item) => item.id), ...(canPlay ? { targetId: target } : {}), ...(action === "respond" ? { providerId: "serpent_spear_attack" } : {}) });
