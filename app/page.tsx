...
export default function Home() {
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "serpent_spear_attack" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_rock_cleaving" | "pass_rock_cleaving" | "use_frost_sword" | "pass_frost_sword" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "choose_target_card" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra: Record<string, unknown> = {}) {
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "serpent_spear_attack" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_rock_cleaving" | "pass_rock_cleaving" | "use_frost_sword" | "pass_frost_sword" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "choose_target_card" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const canRespond = room.phase === "response" && !room.pendingHarvest && !room.pendingTargetCard && !room.pendingFrostSword && room.isMyAction;
  ...
  const greenDragonResponse = canRespond && Boolean(room.pendingGreenDragon);
  const rockCleavingResponse = canRespond && Boolean(room.pendingRockCleaving);
  ...
  const requiredResponseKind = negationResponse ? "Negation" : greenDragonResponse || duelResponse || room.pendingGroup?.requiredKind === "Attack" ? "Attack" : "Dodge";
  const responseCardAllowed = (item: Card) => rockCleavingResponse || (requiredResponseKind === "Negation" ? item.kind === "Negation" : requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge");
  const responsePlayAction = negationResponse ? "respond_negation" : greenDragonResponse ? "respond_green_dragon" : groupResponse ? "respond_group" : duelResponse ? "respond_duel" : "respond_dodge";
  const responseDamageAction = negationResponse ? "pass_negation" : greenDragonResponse ? "pass_green_dragon" : rockCleavingResponse ? "pass_rock_cleaving" : groupResponse ? "take_group_damage" : duelResponse ? "take_duel_damage" : "take_damage";
  const hasSerpentSpear = Boolean(me?.equipmentCards.some((equipment) => equipment.kind === "SerpentSpear"));
  ...
  useEffect(() => { const timer = setTimeout(() => { setSelected(""); setTargetIds([]); setTargetCardIndex(null); setTargetCardZone(""); setTargetCardId(""); setDiscardSelected([]); setSerpentMode(false); setSerpentSelected([]); }, 0); return () => clearTimeout(timer); }, [room.turnSeat, room.phase]);
  useEffect(() => { if (!responseTimerActive) { automaticResponseTimeout.current = ""; return; } if (busy) return; const key = `${room.actionPlayerId}-${room.pendingNegation?.cardName ?? room.pendingGreenDragon?.sourceId ?? room.pendingRockCleaving?.sourceId ?? room.pendingFrostSword?.sourceId ?? room.pendingGroup?.cardKind ?? room.pendingDuel?.sourceId ?? room.pendingAttack?.sourceId ?? "response"}`; if (responseDeadline <= 0) { const startKey = `start-${key}`; if (automaticResponseTimeout.current !== startKey) { automaticResponseTimeout.current = startKey; onActionRef.current("start_response_timer"); } return; } automaticResponseTimeout.current = `timer-${key}-${responseDeadline}`; const timer = setTimeout(() => { automaticResponseTimeout.current = `expired-${key}-${responseDeadline}`; onActionRef.current(frostSwordResponse ? "pass_frost_sword" : responseDamageAction); }, Math.max(0, responseDeadline - Date.now())); return () => clearTimeout(timer); }, [responseTimerActive, frostSwordResponse, busy, responseDeadline, responseDamageAction, room.actionPlayerId, room.pendingNegation?.cardName, room.pendingGreenDragon?.sourceId, room.pendingRockCleaving?.sourceId, room.pendingFrostSword?.sourceId, room.pendingGroup?.cardKind, room.pendingDuel?.sourceId, room.pendingAttack?.sourceId]);
  ...
  const playSerpentAttack = async () => {
    ...
    if (canPlay && (!target || !displayTarget)) return;
    ...
    const action = canPlay ? "serpent_spear_attack" : groupResponse ? "respond_group" : duelResponse ? "respond_duel" : null;
    if (!action) return;
    setSerpentMode(false); setSerpentSelected([]); setSelected(""); setTargetIds([]);
    const accepted = await onAction(action, { cardIds: materials.map((item) => item.id), ...(canPlay ? { targetId: target } : {}) });
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {rockCleavingResponse && <div className="weapon-response-backdrop"><section className="weapon-response-prompt" role="dialog" aria-label="Rock Cleaving Axe decision" aria-live="assertive"><span>ROCK CLEAVING AXE · YOUR RESPONSE</span><b>Your Attack was blocked by Dodge</b><small>Select exactly 2 cards from your hand or Equipment Zone, then force 1 damage — or skip the effect.</small><div><button className="primary" disabled={busy || serpentSelected.length !== 2} onClick={playRockCleaving}>{busy ? "Discarding…" : "Use Rock Cleaving Axe"}</button><button className="end" disabled={busy} onClick={() => onAction("pass_rock_cleaving")}>{busy ? "Skipping…" : "Skip Axe"}</button></div></section></div>}
      {frostSwordResponse && <div className={`weapon-response-backdrop ${frostSelecting ? "frost-selecting-backdrop" : ""}`}><section className="weapon-response-prompt" role="dialog" aria-label="Frost Sword decision" aria-live="assertive"><span>FROST SWORD · YOUR RESPONSE · {frostTarget?.name ?? "TARGET"}</span><b>{frostSelecting ? `Choose ${Math.min(2, frostTarget?.handCount ?? 0) || 1} target card${(frostTarget?.handCount ?? 0) > 1 ? "s" : ""}` : "Choose the Attack result"}</b><small>{frostSelecting ? `Select up to 2 cards from ${frostTarget?.name ?? "the target"}. You choose the cards; the target does not.` : `Prevent the damage and discard up to 2 cards from ${frostTarget?.name ?? "the target"}, or deal 1 damage.`}</small>{frostSelecting && frostTarget && <div className="frost-inline-picker" aria-label={`Choose cards from ${frostTarget.name}`}><strong>{frostTarget.name}&apos;s cards · {frostSelectedKeys.length}/2 selected</strong>{frostTarget.handCount > 0 && <section><small>Hand (hidden)</small><div>{Array.from({ length: frostTarget.handCount }, (_, index) => { const key = `hand:${index}`; return <button type="button" className={frostSelectedKeys.includes(key) ? "selected" : ""} aria-pressed={frostSelectedKeys.includes(key)} key={key} onClick={() => setFrostSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < 2 ? [...keys, key] : keys)}>?</button>; })}</div></section>}{frostTarget.equipmentCards.length > 0 && <section><small>Equipment</small><div>{frostTarget.equipmentCards.map((item) => <button type="button" className={frostSelectedKeys.includes(item.id) ? "selected named" : "named"} aria-pressed={frostSelectedKeys.includes(item.id)} key={item.id} onClick={() => setFrostSelectedKeys((keys) => keys.includes(item.id) ? keys.filter((key) => key !== item.id) : keys.length < 2 ? [...keys, item.id] : keys)}>{cardDefinition(item.kind).name}</button>)}</div></section>}</div>}<div>{frostSelecting ? <><button className="primary" disabled={busy || frostSelectedKeys.length < 1} onClick={() => onAction("use_frost_sword", { cardKeys: frostSelectedKeys })}>{busy ? "Resolving…" : `Discard ${frostSelectedKeys.length} selected`}</button><button className="end" disabled={busy} onClick={() => { setFrostSelecting(false); setFrostSelectedKeys([]); }}>Back</button></> : <><button className="primary" disabled={busy} onClick={() => { setFrostSelecting(true); setFrostSelectedKeys([]); }}>{busy ? "Opening…" : "Choose cards to discard"}</button><button className="end" disabled={busy} onClick={() => onAction("pass_frost_sword")}>{busy ? "Resolving…" : "Deal 1 damage"}</button></>}</div></section></div>}
    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {canChooseTargetCard && <button className="primary" disabled={busy || !targetCardZone || targetCardZone === "hand" && targetCardIndex === null || targetCardZone !== "hand" && !targetCardId} onClick={() => onAction("choose_target_card", { targetCardZone, ...(targetCardZone === "hand" ? { targetCardIndex } : { targetCardId }) })}>{busy ? "Resolving…" : room.pendingTargetCard?.cardKind === "Steal" ? "Obtain selected" : "Discard selected"}</button>}
          {canRespond && <>{rockCleavingResponse ? <button className="primary" disabled={busy || serpentSelected.length !== 2} onClick={playRockCleaving}>{busy ? "Discarding…" : "Use Rock Cleaving Axe"}</button> : <>{canFormSerpentAttack && <button className={`serpent-control ${serpentMode ? "active" : ""}`} onClick={() => { setSerpentMode((active) => !active); setSerpentSelected([]); setSelected(""); }}>{serpentMode ? "Use Attack card" : "Use Serpent Spear"}</button>}<button className="primary" disabled={serpentMode ? busy || serpentSelected.length !== 2 : busy || !card || !responseCardAllowed(card)} onClick={serpentMode ? playSerpentAttack : playResponseCard}>{busy ? "Playing…" : serpentMode ? "Form Attack" : greenDragonResponse ? "Continue Attack" : `Play ${requiredResponseKind}`}</button></>}<button className="end" disabled={busy} onClick={() => onAction(responseDamageAction)}>{busy ? "Skipping…" : negationResponse ? "Skip response" : greenDragonResponse ? "Skip follow-up" : rockCleavingResponse ? "Skip Axe" : "Skip · take 1 damage"}</button></>}
