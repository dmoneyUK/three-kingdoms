...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const activeSkillComplete = Boolean(activeSkillOption && (activeSkillSelection ? activeSkillSelectedCardIds.length >= activeSkillSelection.min && activeSkillSelectedCardIds.length <= activeSkillSelection.max : activeSkillTargetSelection) && (activeSkillTargetIds.length === 0 || targetIds.length === 1 && activeSkillTargetIds.includes(targetIds[0])));
  ...
  useEffect(() => { const timer = setTimeout(() => { setKingSkillId(""); setWushengMode(null); setLongdanMode(null); setResponseProviderId(""); setSelected(""); setTargetIds([]); setSerpentSelected([]); setTriggerSelectedKeys([]); setTriggerChoice(""); }, 0); return () => clearTimeout(timer); }, [room.actionRevision]);
  useEffect(() => {
    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {(activeSkillSelection || activeSkillTargetSelection) && <button className="primary" disabled={busy || presentationBusy || !activeSkillComplete} onClick={() => onAction("trigger", { providerId: activeSkillOption?.effectId, ...(activeSkillSelection ? { cardIds: activeSkillSelectedCardIds } : {}), ...(targetIds.length ? { targetId: targetIds[0] } : {}) })}>{`Use ${activeSkillOption?.label}`}</button>}
