...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <footer className="play-command"><div className="turn-controls"><span>{presentationBusy ? "Showing the current game event…" : room.status === "finished" ? "The match has ended" : room.phase === "dying" ? room.isMyAction ? `Your private Peach decision is open` : "Waiting — no rescue action is required from you" : canRespond ? `Your action · select a Dodge card, then play it—or take damage` : room.phase === "response" ? `Waiting for ${defender?.name ?? "the target"} to answer ${attacker?.name ?? "the attacker"}` : room.phase === "discard" && room.isMyTurn ? `Your action · Discard Phase · select ${excessCards} card${excessCards === 1 ? "" : "s"} (${discardSelected.length}/${excessCards})` : room.isMyTurn ? room.phase === "draw" ? "Your action · Draw Phase" : card?.kind === "Strike" && !target ? "Your action · Play Phase · choose an adjacent target" : room.phase === "play-struck" ? "Your action · Play Phase · Strike used" : "Your action · Play Phase" : `Waiting for ${actor?.name ?? current?.name ?? "another player"}`}</span><div>{canRespond && <><button className="primary" disabled={busy || card?.kind !== "Dodge"} onClick={() => card && onAction("respond_dodge", { cardId: card.id })}>Play selected</button><button className="end" disabled={busy} onClick={() => onAction("take_damage")}>Take 1 damage</button></>}{room.isMyTurn && room.phase === "discard" && <button className="end" disabled={busy || discardSelected.length !== excessCards} onClick={() => onAction("discard_cards", { cardIds: discardSelected })}>Discard {excessCards} selected</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || presentationBusy || !card || (card.kind === "Strike" && (!target || room.phase === "play-struck")) || card.kind === "Dodge"} onClick={play}>Play selected</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("end_turn")}>Finish Play Phase</button></>}</div></div>
      <div className="play-hand">{room.myHand.map((item) => { const definition = cardDefinition(item.kind); const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || canRespond || rescueDecisionReady; return <button key={item.id} disabled={!maySelect || (canRespond && item.kind !== "Dodge") || (rescueDecisionReady && item.kind !== "Peach")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else setSelected((id) => id === item.id ? "" : item.id); setTarget(""); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-glyph">{definition.glyph}</span><strong>{definition.name}</strong><small>{room.phase === "discard" ? isSelected ? "Selected · tap to remove" : "Tap to select" : rescueDecisionReady && item.kind === "Peach" ? isSelected ? "Selected · give this Peach" : "Tap to select this Peach" : canRespond && item.kind === "Dodge" ? isSelected ? "Selected · play it" : "Tap to answer Strike" : definition.description}</small></button>; })}</div>
...
function CardFace({ card }: { card: Card }) {
  ...
  return <div className={`played-card ${card.kind.toLowerCase()}`}><i>{card.rank}<small>{card.suit}</small></i><b>{definition.glyph}</b><strong>{definition.name}</strong></div>;
}
...
function cardEventVerb(event: CardEvent, peachRescue: boolean) {
  ...
  if (event.card.kind === "DrawTwo") return "draws two cards";
  return "attacks";
}

function cardEventHasTarget(event: CardEvent, peachRescue: boolean) {
  return event.action !== "discard" && (event.card.kind === "Strike" || event.card.kind === "Dodge" || peachRescue);
}
...
function cardEventDirection(event: CardEvent, peachRescue: boolean) {
  ...
  if (event.card.kind === "Dodge") return "← BLOCKS";
  if (event.card.kind === "Strike") return "ATTACKS →";
  return "SELF";
}
