...
type GameEvent = (CardEvent & { type: "card"; message?: string }) | { type: "message"; id: string; message: string };
...
type Room = { code: string; status: "lobby" | "heroes" | "started" | "playing" | "finished"; maxPlayers: number; isHost: boolean; meId: string; myRole: string | null; myHeroOptions: Hero[]; players: Player[]; myHand: Card[]; turnSeat: number | null; phase: string | null; deckCount: number; discardTop: Card | null; log: string[]; timeline: GameEvent[]; isMyTurn: boolean; pendingAttack: { sourceId: string; targetId: string } | null; pendingDying: { sourceId: string; targetId: string } | null };
...
export default function Home() {
  ...
  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "rescue_self" | "accept_defeat", extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; token?: string; room?: Room; drawnCards?: Card[] };
      ...
      if (action === "draw" && data.drawnCards?.length) setDrawReveal(data.drawnCards);
  ...
  function leave() {
    localStorage.removeItem("three-realms-session"); setRoom(null); setToken(""); setCode(""); setError(""); setDrawReveal([]);
  }

  if (room?.status === "started" || room?.status === "playing" || room?.status === "finished") return <GameRoom room={room} busy={busy} error={error} drawReveal={drawReveal} onDrawRevealEnd={() => setDrawReveal([])} onAction={send} onLeave={leave} />;
...
function GameRoom({ room, busy, error, drawReveal, onDrawRevealEnd, onAction, onLeave }: { room: Room; busy: boolean; error: string; drawReveal: Card[]; onDrawRevealEnd: () => void; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "rescue_self" | "accept_defeat", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const actionPlayer = room.players.find((player) => player.name === activeCard?.player); const actionTarget = room.players.find((player) => player.name === activeCard?.target);
  ...
  useEffect(() => { const unseenEvents = (room.timeline ?? []).some((event) => !seenEvents.current.has(event.id)); if (room.status !== "playing" || activeEvent || eventQueue.length || unseenEvents || room.turnSeat === previousTurn.current) return; previousTurn.current = room.turnSeat; setTurnNotice(`${current?.name ?? "Player"}'s turn`); const timer = setTimeout(() => setTurnNotice(""), 1600); return () => clearTimeout(timer); }, [room.turnSeat, room.status, current?.name, room.timeline, activeEvent, eventQueue.length]);
  useEffect(() => { const fresh = (room.timeline ?? []).filter((event) => !seenEvents.current.has(event.id)); fresh.forEach((event) => seenEvents.current.add(event.id)); if (fresh.length) setEventQueue((queue) => [...queue, ...fresh]); }, [room.timeline]);
  ...
  useEffect(() => { if (!room.isMyTurn || room.phase !== "draw" || room.status !== "playing" || busy) return; const key = `${room.turnSeat}-${room.timeline.at(-1)?.id ?? "start"}`; if (automaticDraw.current === key) return; automaticDraw.current = key; onAction("draw"); }, [room.isMyTurn, room.phase, room.status, room.turnSeat, room.timeline, busy, onAction]);
  const play = () => { if (!card) return; onAction("play_card", { cardId: card.id, ...(card.kind === "Strike" ? { targetId: target } : {}) }); setSelected(""); setTarget(""); };
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b><span>{current?.name ?? "—"}&apos;s turn · {phaseName(room.phase)}</span></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    <section className="play-table">
      {turnNotice && <div className="turn-notice" role="status"><span>TURN BEGINS</span><b>{turnNotice}</b></div>}
      {drawReveal.length > 0 && <div className="draw-reveal-stage" role="status"><div className="draw-reveal-title"><span>PRIVATE DRAW</span><b>You drew these cards</b><small>Only you can see them</small></div><div className="draw-reveal-cards">{drawReveal.map((drawn) => <div className={`played-card ${drawn.kind.toLowerCase()}`} key={drawn.id}><i>{drawn.rank}<small>{drawn.suit}</small></i><b>{drawn.kind === "Strike" ? "⚔" : drawn.kind === "Dodge" ? "盾" : "桃"}</b><strong>{drawn.kind}</strong></div>)}</div></div>}
      {activeEvent?.type === "message" && <div className="game-event-stage" role="status"><div><span>GAME EVENT</span><b>{activeEvent.message}</b></div></div>}
      {activeCard && <div className="played-card-stage" role="status"><div className="card-action-title"><b>{activeCard.player}</b><span>{activeCard.action === "discard" ? "discards" : activeCard.card.kind === "Dodge" ? "blocks" : activeCard.card.kind === "Peach" ? "heals self" : "attacks"}</span>{activeCard.action !== "discard" && activeCard.card.kind !== "Peach" && <strong>{activeCard.target}</strong>}</div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="action-direction"><span>{activeCard.action === "discard" ? "DISCARD ↓" : activeCard.card.kind === "Peach" ? "SELF" : activeCard.card.kind === "Dodge" ? "← BLOCKS" : "ATTACKS →"}</span><div className={`played-card ${activeCard.card.kind.toLowerCase()}`}><i>{activeCard.card.rank}<small>{activeCard.card.suit}</small></i><b>{activeCard.card.kind === "Strike" ? "⚔" : activeCard.card.kind === "Dodge" ? "盾" : "桃"}</b><strong>{activeCard.card.kind}</strong></div></div>{activeCard.action !== "discard" && activeCard.card.kind !== "Peach" && <ActionHeroCard player={actionTarget} label={activeCard.card.kind === "Dodge" ? "ATTACKER" : "TARGET"} />}</div></div>}
    ...
    <footer className="play-command"><div className="turn-controls"><span>{presentationBusy ? "Showing the current game event… Your controls remain active." : room.status === "finished" ? "The match has ended" : room.phase === "dying" ? room.pendingDying?.targetId === room.meId ? "Dying · use Peach to return to 1 HP, or accept defeat" : `${dyingPlayer?.name ?? "A player"} is deciding whether to use Peach` : room.phase === "response" ? room.pendingAttack?.targetId === room.meId ? `${attacker?.name ?? "An opponent"} attacked you — Dodge prevents all damage` : `${defender?.name ?? "The defender"} is answering ${attacker?.name ?? "the attacker"}'s Strike` : room.phase === "discard" && room.isMyTurn ? `Discard Phase · select ${excessCards} card${excessCards === 1 ? "" : "s"} together (${discardSelected.length}/${excessCards})` : room.isMyTurn ? room.phase === "draw" ? "Draw Phase · drawing two cards automatically…" : card?.kind === "Strike" && !target ? "Play Phase · choose an adjacent opponent (distance 1)" : room.phase === "play-struck" ? "Play Phase · Strike used; play Peach or finish Play" : "Play Phase · use permitted cards in any order" : `Waiting for ${current?.name ?? "another player"}`}</span><div>{room.phase === "dying" && room.pendingDying?.targetId === room.meId && <><button className="primary" disabled={busy || !room.myHand.some((item) => item.kind === "Peach")} onClick={() => onAction("rescue_self")}>Use Peach · Return to 1 HP</button><button className="end" disabled={busy} onClick={() => onAction("accept_defeat")}>No Peach · Accept defeat</button></>}{room.phase === "response" && room.pendingAttack?.targetId === room.meId && <><button className="primary" disabled={busy || !room.myHand.some((item) => item.kind === "Dodge")} onClick={() => onAction("respond_dodge")}>Play Dodge · No damage</button><button className="end" disabled={busy} onClick={() => onAction("take_damage")}>No Dodge · Take damage</button></>}{room.isMyTurn && room.phase === "discard" && <button className="end" disabled={busy || discardSelected.length !== excessCards} onClick={() => onAction("discard_cards", { cardIds: discardSelected })}>Discard {excessCards} selected</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || drawReveal.length > 0 || !card || (card.kind === "Strike" && (!target || room.phase === "play-struck")) || card.kind === "Dodge"} onClick={play}>Play selected</button><button className="end" disabled={busy || drawReveal.length > 0} onClick={() => onAction("end_turn")}>Finish Play Phase</button></>}</div></div>
      <div className="play-hand">{room.myHand.map((item) => { const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : selected === item.id; return <button key={item.id} disabled={!room.isMyTurn || (!canPlay && room.phase !== "discard")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else setSelected((id) => id === item.id ? "" : item.id); setTarget(""); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-glyph">{item.kind === "Strike" ? "⚔" : item.kind === "Dodge" ? "盾" : "桃"}</span><strong>{item.kind}</strong><small>{room.phase === "discard" ? isSelected ? "Selected · tap to remove" : "Tap to select" : item.kind === "Dodge" ? "Play when attacked" : item.kind === "Peach" ? "Recover 1 HP" : "Range 1 · deal 1 damage"}</small></button>; })}</div>
...
function describeEvent(event: GameEvent) {
  ...
  if (event.action === "discard") return `${event.player} reveals and discards ${event.card.rank}${event.card.suit} ${event.card.kind}.`;
  return event.message ?? `${event.player} plays ${event.card.rank}${event.card.suit} ${event.card.kind}${event.target !== event.player ? ` on ${event.target}` : ""}.`;
}
