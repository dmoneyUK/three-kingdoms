    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {triggerResponse && <>{triggerOptions.map((option) => option.selection ? <button key={option.effectId} className={`serpent-control ${responseProviderId === option.effectId ? "active" : ""}`} disabled={responseControlsDisabled} onClick={() => { const active = responseProviderId === option.effectId; setResponseProviderId(active ? "" : option.effectId); setSelected(""); setSerpentSelected([]); setTriggerSelectedKeys([]); }}>{responseProviderId === option.effectId ? `Cancel ${option.label}` : option.label}</button> : <button key={option.effectId} className="primary" disabled={responseControlsDisabled} onClick={() => onAction("trigger", { providerId: option.effectId })}>{busy ? "Resolving…" : option.label}</button>)}{selectedTriggerOption?.selection?.type === "target_cards" && (() => { const target = room.players.find((player) => player.id === selectedTriggerOption.selection.targetId); const eligible = new Set(selectedTriggerOption.selection.eligibleKeys); const eligibleHandIndexes = eligibleHiddenHandIndexes(target?.handCount ?? 0, selectedTriggerOption.selection.eligibleKeys); const eligibleEquipment = target?.equipmentCards.filter((item) => eligible.has(item.id)) ?? []; const max = selectedTriggerOption.selection.max; const toggle = (key: string) => setTriggerSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < max ? [...keys, key] : [...keys, key]); return <div className="hidden-card-picker"><small>Select cards from {target?.name ?? "the target"}</small>{eligibleHandIndexes.length > 0 && <section><small>Hand (hidden)</small><div>{eligibleHandIndexes.map((index) => { const key = `hand:${index}`; return <button type="button" disabled={responseControlsDisabled} className={triggerSelectionKeys.includes(key) ? "selected" : ""} key={key} onClick={() => toggle(key)}>?</button>; })}</div></section>}{eligibleEquipment.length > 0 && <section><small>Equipment</small><div>{eligibleEquipment.map((item) => <button type="button" disabled={responseControlsDisabled} className={triggerSelectionKeys.includes(item.id) ? "selected named" : "named"} key={item.id} onClick={() => toggle(item.id)}>{cardDefinition(item.kind).name}</button>)}</div></section>}</div>; })()}{(selectedTriggerOption?.selection?.type === "cards" || selectedTriggerOption?.selection?.type === "target_cards") && <button className="primary" disabled={responseControlsDisabled || !triggerSelectionComplete} onClick={() => onAction("trigger", { providerId: selectedTriggerOption.effectId, ...(triggerSelectionUsesTargetCards ? { cardKeys: triggerSelectionKeys } : triggerSelectedCardIds.length === 1 ? { cardId: triggerSelectedCardIds[0] } : { cardIds: triggerSelectedCardIds }) })}>{busy ? "Resolving…" : `Use ${selectedTriggerOption.label}`}</button>}{triggerDeclineAction && <button className="end" disabled={responseControlsDisabled} onClick={() => onAction("decline_trigger")}>{busy ? "Skipping…" : "Skip reaction"}</button>}</>}
