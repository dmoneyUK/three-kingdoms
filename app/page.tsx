...
function pendingTimelineSequence(room: Room) {
  const sourceId = room.pendingNegation?.sourceId ?? room.pendingDuel?.sourceId ?? room.pendingAttack?.sourceId ?? room.pendingGreenDragon?.sourceId ?? room.pendingGroup?.sourceId;
  const source = room.players.find((player) => player.id === sourceId);
  if (!source) return [];
  const expectedName = room.pendingNegation?.cardName ?? (room.pendingDuel ? "Duel" : room.pendingGroup ? cardDefinition(room.pendingGroup.cardKind).name : room.pendingAttack || room.pendingGreenDragon ? "Attack" : null);
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "serpent_spear_attack" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  useEffect(() => { const timer = setTimeout(() => { setSelected(""); setTarget(""); setTargetCardIndex(null); setDiscardSelected([]); setSerpentMode(false); setSerpentSelected([]); }, 0); return () => clearTimeout(timer); }, [room.turnSeat, room.phase]);
