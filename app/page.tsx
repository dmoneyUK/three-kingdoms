...
type PresentationEventMeta = { resolutionId?: string; importance?: PresentationImportance; finalResult?: boolean; playedAs?: "attack"; effectNotice?: boolean };
...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const [selected, setSelected] = useState(""); const [playAsAttack, setPlayAsAttack] = useState(false); const [wushengMode, setWushengMode] = useState<"play" | "response" | null>(null); const [targetIds, setTargetIds] = useState<string[]>([]); const target = targetIds[0] ?? "";
  ...
  // Compact board compatibility keeps the player-equipment-zone and player-judgement-zone semantics.
  // Equipment cards retain the player-equipment-card presentation contract.
  ...
  const triggerSelectionKeys = triggerSelectionUsesTargetCards || triggerSelectionUsesChoice ? triggerSelectedKeys : [];
  const triggerSelectionComplete = Boolean(triggerSelection && ((triggerSelectionUsesCards && triggerSelectedCardIds.length >= triggerSelection.min && triggerSelectedCardIds.length <= triggerSelection.max) || (triggerSelectionUsesTargetCards && triggerSelectionKeys.length >= triggerSelection.min && triggerSelectionKeys.length <= triggerSelection.max) || (triggerSelectionUsesChoice && triggerChoice === "draw" || triggerSelectionUsesChoice && triggerChoice === "discard" && triggerSelectionKeys.length === 1)));
  ...
  const selectedCanPlayAsAttack = Boolean(card && (isAttackCard(card) || wushengMode === "play" && playPhaseAttackCardIds.has(card.id) || playAsAttack && playPhaseAttackCardIds.has(card.id)));
  ...
  useEffect(() => { const timer = setTimeout(() => { setWushengMode(null); setResponseProviderId(""); setSelected(""); setSerpentSelected([]); }, 0); return () => clearTimeout(timer); }, [room.actionRevision]);
  useEffect(() => {
  ...
  const chooseBorrowedSwordTarget = (playerId: string) => { if (!canChooseBorrowedSword || !room.pendingBorrowedSword?.eligibleTargetIds.includes(playerId)) return; void onAction("choose_borrowed_sword_target", { targetId: playerId }); };
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {room.isMyTurn && canPlay && !serpentMode && card && !isAttackCard(card) && playPhaseAttackCardIds.has(card.id) && <button className="serpent-control" disabled={busy || presentationBusy || !canDeclareAttack} onClick={() => { setPlayAsAttack((active) => !active); setTargetIds([]); }}>{playAsAttack ? "Play natively" : "Use as Attack"}</button>}
      ...
      {canChooseBorrowedSword && room.pendingBorrowedSword && <div className="borrowed-sword-picker" role="dialog" aria-label="Borrowed Sword target selection"><span>Choose target for the forced Attack</span><div>{room.players.map((player) => { const eligible = room.pendingBorrowedSword?.eligibleTargetIds.includes(player.id) ?? false; return <button type="button" key={player.id} className={eligible ? "eligible" : ""} disabled={!eligible || busy} onClick={() => chooseBorrowedSwordTarget(player.id)}><strong>{player.name}</strong><small>{heroName(player.hero)}</small></button>; })}</div></div>}
      ...
      {room.players.map((player, index) => { if (!player.equipmentCards.length) return null; const relativeIndex = (index - myTableIndex + room.players.length) % room.players.length; const angle = 180 + (360 / room.players.length) * relativeIndex; return <aside className={`zone-info-overlay equipment-info-overlay equipment-info-overlay-${relativeIndex}`} key={`equipment-info-${player.id}`} style={{ "--angle": `${angle}deg` } as React.CSSProperties}>{player.equipmentCards.map((equipment) => <button type="button" className="card-info-button zone-card-info-button" aria-label={`Explain ${cardDefinition(equipment.kind).name}`} key={equipment.id} onClick={() => setInfoCard(equipment)}>i</button>)}</aside>; })}
      {room.players.map((player, index) => { if (!player.judgementCards.length) return null; const relativeIndex = (index - myTableIndex + room.players.length) % room.players.length; const angle = 180 + (360 / room.players.length) * relativeIndex; return <aside className="zone-info-overlay judgement-info-overlay" key={`judgement-info-${player.id}`} style={{ "--angle": `${angle}deg` } as React.CSSProperties}>{player.judgementCards.map((judgement) => <button type="button" className="card-info-button zone-card-info-button" aria-label={`Explain ${cardDefinition(judgement.kind).name}`} key={judgement.id} onClick={() => setInfoCard(judgement)}>i</button>)}</aside>; })}
      ...
      <div className="player-board" aria-label="Players">{room.players.map((player, index) => { const relativeIndex = (index - myTableIndex + room.players.length) % room.players.length; const targetablePlayer = Boolean((serpentMode && canPlay) || (card && (selectedCanPlayAsAttack || card.kind === "Dismantle" || card.kind === "Steal" || card.kind === "Duel" || card.kind === "BorrowedSword" || card.kind === "Overindulgence" || card.kind === "RationsDepleted"))); const miniEquipment = player.equipmentCards.map((equipment) => <span className="mini-zone-card" key={equipment.id}><button type="button" className="mini-equipment-button" disabled={!(triggerResponse && triggerSelectionUsesCards) || !responseDecisionReady || player.id !== room.meId || Boolean(triggerCardOption && !triggerCardOption.selection?.eligibleCardIds.includes(equipment.id))} onClick={() => setSerpentSelected((ids) => ids.includes(equipment.id) ? ids.filter((id) => id !== equipment.id) : ids.length < (triggerResponse && triggerSelectionUsesCards ? triggerSelection.max : 2) ? [...ids, equipment.id] : ids)}><small>{equipment.rank}{equipment.suit}</small><b>{cardDefinition(equipment.kind).name}</b></button><button type="button" className="zone-info-button" aria-label={`Explain ${cardDefinition(equipment.kind).name}`} onClick={(event) => { event.stopPropagation(); setInfoCard(equipment); }}>i</button></span>); const miniJudgement = player.judgementCards.map((judgement) => <span className="mini-zone-card judgement-mini" key={judgement.id}><span><small>{judgement.rank}{judgement.suit}</small><b>{cardDefinition(judgement.kind).name}</b></span><button type="button" className="zone-info-button" aria-label={`Explain ${cardDefinition(judgement.kind).name}`} onClick={() => setInfoCard(judgement)}>i</button></span>); return <article className={`player-square player-square-${relativeIndex} ${player.id === room.meId ? "self-square" : ""} ${player.seat === room.turnSeat ? "turn-square" : ""} ${player.id === room.actionPlayerId ? "action-square" : ""} ${!player.alive ? "defeated-square" : ""}`} key={`square-${player.id}`}><button type="button" className="player-square-target" disabled={!room.isMyTurn || !canPlay || !player.alive || player.id === room.meId || !targetablePlayer} onClick={() => { setTarget(player.id); setTargetCardIndex(null); }}><strong>{player.name}</strong><span>{heroName(player.hero)}</span>{player.id === room.meId && player.role && <em className="private-role">Role · {player.role}</em>}<small>{hpDisplay(player.hp)} · {player.hp ?? 0}/{player.maxHp ?? 0} HP · {player.handCount} cards</small></button>{player.equipmentCards.length > 0 && <div className="square-zone"><label>Equipment</label><div>{miniEquipment}</div></div>}{player.judgementCards.length > 0 && <div className="square-zone judgement-square-zone"><label>Judgement</label><div>{miniJudgement}</div></div>}</article>; })}</div>
    ...
    {triggerResponse && selectedTriggerOption?.selection?.type === "choice" && <div className="choice-decision" role="dialog" aria-label="Yin-Yang Swords decision"><strong>{selectedTriggerOption.label}</strong><span>Choose one:</span>{selectedTriggerOption.selection.choices.map((choice) => <button type="button" key={choice.id} className={triggerChoice === choice.id ? "selected" : ""} disabled={responseControlsDisabled} onClick={() => { setTriggerChoice(choice.id); setTriggerSelectedKeys([]); }}>{choice.label}</button>)}{triggerChoice === "discard" && <div className="hidden-card-picker"><small>Choose one hand card to discard</small><div>{selectedTriggerOption.selection.eligibleHandKeys.map((key) => { const index = Number(key.split(":")[1]); return <button type="button" key={key} className={triggerSelectionKeys.includes(key) ? "selected" : ""} disabled={responseControlsDisabled} onClick={() => setTriggerSelectedKeys([key])}>Card {index + 1}</button>; })}</div></div>}<button className="primary" disabled={responseControlsDisabled || !triggerSelectionComplete} onClick={() => onAction("trigger", { providerId: selectedTriggerOption.effectId, choice: triggerChoice, ...(triggerChoice === "discard" ? { cardKeys: triggerSelectionKeys } : {}) })}>Confirm choice</button></div>}
    <footer className="play-command">
      <div className="turn-controls">
        <span>{commandPrompt}</span>
        <div>
          <section className="hero-skills" aria-label="Hero Skills">
            ...
            {me?.hero === "guan-yu" && <button type="button" className={`hero-skill-button ${wushengMode ? "active" : ""}`} disabled={wushengButtonDisabled} onClick={() => {
              ...
              if (canUseWushengInPlay) { setWushengMode("play"); setPlayAsAttack(false); setSerpentMode(false); setSelected(""); setTargetIds([]); return; }
              if (canUseWushengInResponse && wushengResponseOption) { setWushengMode("response"); setResponseProviderId(wushengResponseOption.providerId); setPlayAsAttack(false); setSelected(""); setSerpentSelected([]); }
            }}>{wushengMode ? "Cancel Wusheng" : "Wusheng"}</button>}
          </section>
      ...
      <div className="play-hand">{room.myHand.map((item) => { const definition = cardDefinition(item.kind); const costSelection = serpentMode || canRespond && responseSelectionUsesCards && responseSelectionMax > 1 || triggerResponse && triggerSelectionUsesCards && triggerSelectionMax > 1; const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : costSelection ? serpentSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || responseDecisionReady || rescueDecisionReady; const responseSelectionLimit = triggerResponse && triggerSelectionUsesCards ? triggerSelection.max : responseSelectionUsesCards ? responseSelection.max : 2; const wushengCardDisabled = Boolean(wushengMode && !wushengEligibleCardIds.has(item.id)); return <div className="card-slot" key={item.id}><button disabled={!maySelect || wushengCardDisabled || (responseDecisionReady && (canRespond || triggerResponse) && !responseCardAllowed(item)) || (rescueDecisionReady && item.kind !== "Peach")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else if (costSelection) setSerpentSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < responseSelectionLimit ? [...ids, item.id] : ids); else { setSelected((id) => id === item.id ? "" : item.id); setTarget(""); } setTargetCardIndex(null); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""} ${wushengMode && wushengEligibleCardIds.has(item.id) ? "hero-skill-eligible" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-name-mark">{definition.name}</span><strong>{definition.category} card</strong></button><button type="button" className="card-info-button" aria-label={`Explain ${definition.name}`} onClick={() => setInfoCard(item)}>i</button></div>; })}</div>
