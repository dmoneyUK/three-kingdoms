...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const selectedResponseProvider = semanticResponseOptions.find((option) => option.providerId === responseProviderId) ?? null;
  ...
  const commandPrompt = presentationBusy && (canRespond || frostSwordResponse || !canRescue) ? "Showing the current game event…"
    ...
    : canRespond ? rockCleavingResponse ? `Your action · choose 2 cards for Rock Cleaving Axe (${serpentSelected.length}/2), or skip` : selectedResponseProvider && responseSelectionUsesCards ? `Your action · choose ${responseSelection.min === responseSelection.max ? responseSelection.min : `${responseSelection.min}-${responseSelection.max}`} card${responseSelection.max === 1 ? "" : "s"} for ${selectedResponseProvider.label} (${responseSelectedCardIds.length}/${responseSelection.max})` : negationResponse ? `Your action · choose how to Negate ${room.pendingNegation?.responseTarget ?? room.pendingNegation?.cardName}, or skip` : greenDragonResponse ? "Your action · Green Dragon Blade may play another Attack on the same target, or skip" : `Your action · choose how to provide ${requiredResponseKind} for ${room.pendingGroup ? room.pendingGroup.cardKind === "SkyPiercingHalberdAttack" ? "Sky Piercing Halberd Attack" : cardDefinition(room.pendingGroup.cardKind).name : room.pendingDuel ? "the Duel" : "the Attack"}, or skip and take damage`
