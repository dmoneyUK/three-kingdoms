...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {triggerResponse && responseDecisionReady && targetCardPickerOption && targetCardPickerSelection && targetCardPickerTarget && <TargetCardPicker option={targetCardPickerOption} selection={targetCardPickerSelection} target={targetCardPickerTarget} selectedKeys={triggerSelectedKeys} disabled={responseControlsDisabled} busy={busy} canDecline={triggerDeclineAction} error={error} onToggle={(key) => setTriggerSelectedKeys((keys) => keys.includes(key) ? keys.filter((item) => item !== key) : keys.length < targetCardPickerSelection.max ? [...keys, key] : keys)} onUse={() => onAction("trigger", { providerId: targetCardPickerOption.effectId, cardKeys: triggerSelectedKeys })} onDecline={() => onAction("decline_trigger")} />}
...
function TargetCardPicker({ option, selection, target, selectedKeys, disabled, busy, canDecline, error, onToggle, onUse, onDecline }: { option: TriggerOptionView; selection: TargetCardSelection; target: Player; selectedKeys: string[]; disabled: boolean; busy: boolean; canDecline: boolean; error: string; onToggle: (key: string) => void; onUse: () => void; onDecline: () => void }) {
  ...
  const mountOnly = items.length > 0 && items.every((item) => item.card && ["offensiveHorse", "defensiveHorse"].includes(cardDefinition(item.card.kind).equipmentSlot ?? ""));
  const subtitle = mountOnly ? `Choose ${amount} Mount${selection.max === 1 ? "" : "s"} to discard` : `Choose ${amount} card${selection.max === 1 ? "" : "s"} to discard instead of dealing damage`;
  const complete = selectedKeys.length >= selection.min && selectedKeys.length <= selection.max;
  ...
  return <div className="target-card-picker-overlay" role="presentation"><section className="target-card-picker-panel" role="dialog" aria-modal="true" aria-label={`${effectLabel} target card selection`}>
    ...
    <div className="target-card-picker-card-row" aria-label="Eligible cards">
      {items.map((item) => <button type="button" key={item.key} className={`target-card-picker-card ${item.hidden ? "hidden" : "equipment"} ${selectedKeys.includes(item.key) ? "selected" : ""}`} disabled={disabled} aria-pressed={selectedKeys.includes(item.key)} aria-label={item.label} onClick={() => onToggle(item.key)}>{item.hidden ? <span aria-hidden="true">?</span> : item.card && <CardFace card={item.card} />}{selectedKeys.includes(item.key) && <span className="target-card-picker-check" aria-hidden="true">✓</span>}</button>)}
    </div>
    <div className="target-card-picker-count" aria-live="polite">{selectedKeys.length} / {selection.max} selected</div>
    <div className="target-card-picker-actions">
      ...
      <button type="button" className="primary" disabled={disabled || !complete} onClick={onUse}>{busy ? "Resolving…" : `Use ${effectLabel}`}</button>
    </div>
    {error && <p className="error" role="alert">{error}</p>}
  </section></div>;
}
