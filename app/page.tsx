...
type CardEvent = { id: string; player: string; target: string; card: Card; action?: "play" | "discard" | "gain"; presentation?: boolean };
...
type Room = { code: string; status: "lobby" | "heroes" | "started" | "playing" | "finished"; maxPlayers: number; isHost: boolean; meId: string; myRole: string | null; myHeroOptions: Hero[]; players: Player[]; myHand: Card[]; turnSeat: number | null; phase: string | null; deckCount: number; discardTop: Card | null; log: string[]; timeline: GameEvent[]; isMyTurn: boolean; actionPlayerId: string | null; actionReason: string; isMyAction: boolean; pendingAttack: { sourceId: string; targetId: string } | null; pendingDuel: { sourceId: string; targetId: string; actorId: string; opponentId: string } | null; pendingGroup: { cardKind: "BarbarianInvasion" | "RainingArrows"; sourceId: string; actorId: string; requiredKind: "Attack" | "Dodge" } | null; pendingHarvest: { sourceId: string; actorId: string; revealed: Card[]; availableIds: string[]; choices: { cardId: string; playerId: string; playerName: string }[]; previewCardId: string | null; complete: boolean } | null; pendingDying: { sourceId: string; targetId: string; deadline: number } | null };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => fetchRoom(roomCode, token, true), room?.pendingHarvest ? 300 : 2500);
    return () => clearInterval(timer);
  }, [roomCode, token, busy, fetchRoom, room?.pendingHarvest]);

  async function send(action: "create" | "join" | "start" | "add_test_players" | "choose_hero" | "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra: Record<string, unknown> = {}) {
    const epoch = ++stateEpoch.current; const backgroundPreview = action === "preview_harvest";
    if (!backgroundPreview) { setBusy(true); setError(""); }
    try {
    ...
    finally { if (!backgroundPreview) setBusy(false); }
  }
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const shownCard = optimisticPlay ?? activeCard;
  ...
  const peachRescue = shownCard?.card.kind === "Peach" && shownCard.target !== shownCard.player;
  ...
  const actionPlayer = room.players.find((player) => player.name === (shownCard?.player ?? activeCards?.player)); const actionTarget = room.players.find((player) => player.name === shownCard?.target);
  ...
  const requiredResponseKind = duelResponse || room.pendingGroup?.requiredKind === "Attack" ? "Attack" : "Dodge";
  const responseCardAllowed = (item: Card) => requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge";
  const responsePlayAction = groupResponse ? "respond_group" : duelResponse ? "respond_duel" : "respond_dodge";
  const responseDamageAction = groupResponse ? "take_group_damage" : duelResponse ? "take_duel_damage" : "take_damage";
  ...
  useEffect(() => { if (!canRespond) { automaticDamage.current = ""; return; } const requiredKind = room.pendingDuel || room.pendingGroup?.requiredKind === "Attack" ? "Attack" : "Dodge"; if (busy || room.myHand.some((item) => requiredKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge")) return; const key = room.pendingGroup ? `group-${room.pendingGroup.cardKind}-${room.pendingGroup.actorId}` : room.pendingDuel ? `duel-${room.pendingDuel.actorId}-${room.pendingDuel.opponentId}` : `attack-${room.pendingAttack?.sourceId}-${room.pendingAttack?.targetId}`; if (automaticDamage.current === key) return; automaticDamage.current = key; onActionRef.current(room.pendingGroup ? "take_group_damage" : room.pendingDuel ? "take_duel_damage" : "take_damage"); }, [canRespond, busy, room.myHand, room.pendingAttack, room.pendingDuel, room.pendingGroup]);
  ...
  const play = async () => { if (!card || !me) return; const playedCard = card; const needsTarget = isAttackCard(card) || card.kind === "Dismantle" || card.kind === "Steal" || card.kind === "Duel"; const displayTarget = targetPlayer?.name ?? (card.kind === "BumperHarvest" || card.kind === "Oath" ? "All living players" : card.kind === "BarbarianInvasion" || card.kind === "RainingArrows" ? "All other players" : me.name); setOptimisticPlay({ id: `optimistic-${card.id}`, type: "card", player: me.name, target: displayTarget, card, action: "play" }); setSelected(""); setTarget(""); setTargetCardIndex(null); const accepted = await onAction("play_card", { cardId: playedCard.id, ...(needsTarget ? { targetId: target } : {}), ...(["Dismantle", "Steal"].includes(playedCard.kind) ? { targetCardIndex } : {}) }); if (!accepted) setOptimisticPlay(null); };
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className="play-table">
      ...
      {shownCard && <div className="played-card-stage" role="status"><div className="card-action-title"><b>{shownCard.player}</b><span>{cardEventVerb(shownCard, peachRescue)}</span>{cardEventHasTarget(shownCard, peachRescue) && <strong>{shownCard.target}</strong>}</div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="action-direction"><span>{cardEventDirection(shownCard, peachRescue)}</span><CardFace card={shownCard.card} /></div>{cardEventHasTarget(shownCard, peachRescue) && <ActionHeroCard player={actionTarget} label={shownCard.card.kind === "Dodge" ? "ATTACKER" : peachRescue ? "DYING PLAYER" : "TARGET"} />}</div></div>}
      {activeCards && <div className="played-card-stage discard-group-stage" role="status"><div className="card-action-title"><b>{activeCards.player}</b><span>{activeCards.action === "reveal" ? "reveals for Bumper Harvest" : "discards together"}</span></div><div className="card-battle-row"><ActionHeroCard player={actionPlayer} label="PLAYER" /><div className="discard-card-row">{activeCards.cards.map((shown) => <CardFace card={shown} key={shown.id} />)}</div></div></div>}
      {room.pendingHarvest && !presentationBusy && <div className="game-event-stage harvest-choice-stage" role="dialog" aria-label="Bumper Harvest card choice"><div><span>BUMPER HARVEST</span><b>{room.pendingHarvest.complete ? "All choices complete" : canChooseHarvest ? "Your turn — choose one card" : `${actor?.name ?? "The next player"} is choosing`}</b><small>{room.pendingHarvest.complete ? "The final shaded card remains visible before Bumper Harvest closes." : canChooseHarvest ? "Tap any available card to change your selection, then confirm. Selection changes are instant." : "Watch the current player's card rise, then become shaded when confirmed."}</small><div className="harvest-card-row">{room.pendingHarvest.revealed.map((choice) => { const takenBy = room.pendingHarvest?.choices.find((entry) => entry.cardId === choice.id); const available = room.pendingHarvest?.availableIds.includes(choice.id); const awaitingConfirmation = activeHarvestSelection === choice.id; return <button type="button" className={`harvest-card-choice ${takenBy ? "taken" : ""} ${awaitingConfirmation ? "pending-choice" : ""}`} disabled={!canChooseHarvest || busy || !available} aria-pressed={awaitingConfirmation} aria-label={takenBy ? `${cardDefinition(choice.kind).name}, taken by ${takenBy.playerName}` : `${cardDefinition(choice.kind).name}, ${awaitingConfirmation ? `selected by ${actor?.name ?? "current player"}, awaiting confirmation` : "available"}`} key={choice.id} onClick={() => { const nextCardId = activeHarvestSelection === choice.id ? "" : choice.id; setHarvestSelected(nextCardId); void publishHarvestPreview(nextCardId); }}><CardFace card={choice} />{takenBy && <strong className="harvest-taken-label">Taken by {takenBy.playerName}</strong>}{awaitingConfirmation && <strong className="harvest-pending-label">Selected by {actor?.name ?? "player"}</strong>}</button>; })}</div>{canChooseHarvest && <div className="harvest-confirm-row"><small>{harvestSelectedCard ? `${cardDefinition(harvestSelectedCard.kind).name} selected` : "Select a card before confirming"}</small><button type="button" className="primary" disabled={busy || !harvestSelectedCard} onClick={() => { if (!harvestSelectedCard) return; queuedHarvestPreview.current = null; onAction("choose_harvest", { cardId: harvestSelectedCard.id }); setHarvestSelected(""); }}>{busy ? "Confirming…" : "Confirm choice"}</button></div>}</div></div>}
    ...
    <footer className="play-command"><div className="turn-controls"><span>{presentationBusy ? "Showing the current game event…" : room.status === "finished" ? "The match has ended" : room.phase === "dying" ? room.isMyAction ? `Your private Peach decision is open` : "Waiting — no rescue action is required from you" : room.pendingHarvest ? room.pendingHarvest.complete ? "Bumper Harvest · showing all confirmed choices" : canChooseHarvest ? "Your action · choose one revealed Bumper Harvest card" : `Waiting for ${actor?.name ?? "the next player"} to choose from Bumper Harvest` : canRespond ? `Your action · select ${requiredResponseKind} for ${room.pendingGroup ? cardDefinition(room.pendingGroup.cardKind).name : room.pendingDuel ? "the Duel" : "the Attack"}—or take damage` : room.phase === "response" ? room.pendingGroup ? `Waiting for ${actor?.name ?? "the target"} to play ${room.pendingGroup.requiredKind}` : room.pendingDuel ? `Waiting for ${actor?.name ?? "the duelist"} to play Attack` : `Waiting for ${defender?.name ?? "the target"} to answer ${attacker?.name ?? "the attacker"}` : room.phase === "discard" && room.isMyTurn ? `Your action · Discard Phase · select ${excessCards} card${excessCards === 1 ? "" : "s"} (${discardSelected.length}/${excessCards})` : room.isMyTurn ? room.phase === "draw" ? "Your action · Draw Phase" : card && isAttackCard(card) && !target ? "Your action · Play Phase · choose an adjacent target" : card?.kind === "Dismantle" && !target ? "Your action · choose a player with cards" : card?.kind === "Steal" && !target ? "Your action · choose a player within distance 1" : card?.kind === "Duel" && !target ? "Your action · choose any other player" : ["Dismantle", "Steal"].includes(card?.kind ?? "") && targetCardIndex === null ? "Your action · choose one hidden card" : room.phase === "play-struck" ? "Your action · Play Phase · Attack used" : "Your action · Play Phase" : `Waiting for ${actor?.name ?? current?.name ?? "another player"}`}</span><div>{canRespond && <><button className="primary" disabled={busy || !card || !responseCardAllowed(card)} onClick={() => card && onAction(responsePlayAction, { cardId: card.id })}>{busy ? "Playing…" : `Play ${requiredResponseKind}`}</button><button className="end" disabled={busy} onClick={() => onAction(responseDamageAction)}>{busy ? "Applying…" : "Take 1 damage"}</button></>}{room.isMyTurn && room.phase === "discard" && <button className="end" disabled={busy || discardSelected.length !== excessCards} onClick={() => onAction("discard_cards", { cardIds: discardSelected })}>{busy ? "Discarding…" : `Discard ${excessCards} selected`}</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || presentationBusy || !card || (isAttackCard(card) && (!target || room.phase === "play-struck")) || (["Dismantle", "Steal"].includes(card.kind) && (!target || targetCardIndex === null)) || (card.kind === "Duel" && !target) || card.kind === "Dodge"} onClick={play}>{busy ? "Playing…" : "Play selected"}</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("end_turn")}>{busy ? "Finishing…" : "Finish Play Phase"}</button></>}</div></div>
      {["Dismantle", "Steal"].includes(card?.kind ?? "") && targetPlayer && <div className="hidden-card-picker" aria-label={`Choose one hidden card from ${targetPlayer.name}`}><span>{targetPlayer.name}&apos;s hand</span>{Array.from({ length: targetPlayer.handCount }, (_, index) => <button type="button" className={targetCardIndex === index ? "selected" : ""} aria-pressed={targetCardIndex === index} key={index} onClick={() => setTargetCardIndex(index)}>?</button>)}</div>}
      <div className="play-hand">{room.myHand.map((item) => { const definition = cardDefinition(item.kind); const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : selected === item.id; const maySelect = (room.isMyTurn && (canPlay || room.phase === "discard")) || canRespond || rescueDecisionReady; return <div className="card-slot" key={item.id}><button disabled={!maySelect || (canRespond && !responseCardAllowed(item)) || (rescueDecisionReady && item.kind !== "Peach")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else setSelected((id) => id === item.id ? "" : item.id); setTarget(""); setTargetCardIndex(null); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-name-mark">{definition.name}</span><strong>{definition.category} card</strong><small>{room.phase === "discard" ? isSelected ? "Selected · tap to remove" : "Tap to select" : rescueDecisionReady && item.kind === "Peach" ? isSelected ? "Selected · give this Peach" : "Tap to select this Peach" : canRespond && responseCardAllowed(item) ? isSelected ? "Selected · play it" : room.pendingGroup ? `Tap to answer ${cardDefinition(room.pendingGroup.cardKind).name}` : duelResponse ? "Tap to answer Duel" : "Tap to answer Attack" : definition.description}</small></button><button type="button" className="card-info-button" aria-label={`Explain ${definition.name}`} onClick={() => setInfoCard(item)}>i</button></div>; })}</div>
