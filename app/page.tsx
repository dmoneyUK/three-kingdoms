...
type GameEvent = (CardEvent & { type: "card"; message?: string }) | { type: "message"; id: string; message: string; presentation?: boolean };
...
function GameRoom({ room, busy, error, drawReveal, onDrawRevealEnd, onAction, onLeave }: { room: Room; busy: boolean; error: string; drawReveal: Card[]; onDrawRevealEnd: () => void; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "rescue_self" | "accept_defeat", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const [turnNotice, setTurnNotice] = useState(""); const previousTurn = useRef(""); const readyTurn = useRef("");
  ...
  useEffect(() => { const unseenEvents = (room.timeline ?? []).some((event) => !seenEvents.current.has(event.id)); const turnKey = `${room.turnSeat}-${room.timeline.at(-1)?.id ?? "start"}`; if (room.status !== "playing" || room.phase !== "draw" || activeEvent || eventQueue.length || unseenEvents || turnKey === previousTurn.current) return; previousTurn.current = turnKey; setTurnNotice(`${current?.name ?? "Player"}'s turn`); const timer = setTimeout(() => { readyTurn.current = turnKey; setTurnNotice(""); }, 1600); return () => clearTimeout(timer); }, [room.turnSeat, room.phase, room.status, current?.name, room.timeline, activeEvent, eventQueue.length]);
  ...
  useEffect(() => { const key = `${room.turnSeat}-${room.timeline.at(-1)?.id ?? "start"}`; if (!room.isMyTurn || room.phase !== "draw" || room.status !== "playing" || busy || presentationBusy || drawReveal.length || readyTurn.current !== key) return; if (automaticDraw.current === key) return; automaticDraw.current = key; onAction("draw"); }, [room.isMyTurn, room.phase, room.status, room.turnSeat, room.timeline, busy, presentationBusy, drawReveal.length, onAction]);
