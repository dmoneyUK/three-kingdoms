...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const responseCardOptionFor = (item: Card | undefined) => item ? semanticResponseOptions.find((option) => option.selection?.type === "cards" && option.selection.min === 1 && option.selection.max === 1 && option.selection.eligibleCardIds.includes(item.id)) : undefined;
  const selectedResponseCardOption = responseCardOptionFor(card);
  ...
  const responseCardAllowed = (item: Card) => responseCardOptionFor(item) !== undefined || !semanticResponseOptions.length && Boolean(requiredResponseKind) && (rockCleavingResponse || (requiredResponseKind === "Negation" ? item.kind === "Negation" : requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge"));
  const canUseEightTrigrams = Boolean(eightTrigramsResponseOption) || canUseAction(room.currentAction, "respond_eight_trigrams");
  const responsePlayAction = genericResponse && selectedResponseCardOption ? "respond" as GameplayAction : (["respond_negation", "respond_green_dragon", "respond_group", "respond_duel", "respond_dodge"] as GameplayAction[]).find((action) => canUseAction(room.currentAction, action)) ?? null;
    ...
    <footer className="play-command">
      <div className="turn-controls">
        ...
        <div>
          ...
          {canRespond && <>{rockCleavingResponse ? <button className="primary" disabled={responseControlsDisabled || serpentSelected.length !== 2} onClick={playRockCleaving}>{busy ? "Discarding…" : "Use Rock Cleaving Axe"}</button> : <>{canFormSerpentAttack && <button className={`serpent-control ${serpentMode ? "active" : ""}`} disabled={responseControlsDisabled} onClick={() => { setSerpentMode((active) => !active); setSerpentSelected([]); setSelected(""); }}>{serpentMode ? "Use Attack card" : "Use Serpent Spear"}</button>}<button className="primary" disabled={serpentMode ? responseControlsDisabled || serpentSelected.length !== 2 : responseControlsDisabled || !card || !responseCardAllowed(card)} onClick={serpentMode ? playSerpentAttack : playResponseCard}>{busy ? "Playing…" : serpentMode ? "Form Attack" : greenDragonResponse ? "Continue Attack" : `Play ${requiredResponseKind}`}</button>{canUseEightTrigrams && <button className="primary" disabled={responseControlsDisabled} onClick={() => onAction(eightTrigramsResponseOption ? "respond" : "respond_eight_trigrams", eightTrigramsResponseOption ? { providerId: "eight_trigrams_dodge" } : {})}>{busy ? "Judging…" : "Use Eight Trigrams"}</button>}</>}<button className="end" disabled={responseControlsDisabled || !responseDamageAction} onClick={() => responseDamageAction && onAction(responseDamageAction)}>{busy ? "Skipping…" : negationResponse ? "Skip response" : greenDragonResponse ? "Skip follow-up" : rockCleavingResponse ? "Skip Axe" : "Skip · take 1 damage"}</button></>}
