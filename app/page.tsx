...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  const canChooseHarvest = room.phase === "response" && Boolean(room.pendingHarvest) && room.isMyAction;
  ...
  useEffect(() => { const fresh = (room.timeline ?? []).filter((event) => !seenEvents.current.has(event.id)); fresh.forEach((event) => seenEvents.current.add(event.id)); const visible = fresh.filter((event) => event.presentation !== false); if (visible.length) setEventQueue((queue) => [...queue, ...visible]); setProcessedTimelineKey(timelineKey); }, [room.timeline, timelineKey]);
  ...
  const play = () => { if (!card) return; const needsTarget = isAttackCard(card) || card.kind === "Dismantle" || card.kind === "Steal" || card.kind === "Duel"; onAction("play_card", { cardId: card.id, ...(needsTarget ? { targetId: target } : {}), ...(["Dismantle", "Steal"].includes(card.kind) ? { targetCardIndex } : {}) }); setSelected(""); setTarget(""); setTargetCardIndex(null); };
