...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const triggerSelectionComplete = Boolean(triggerSelection && ((triggerSelectionUsesCards && triggerSelectedCardIds.length >= triggerSelection.min && triggerSelectedCardIds.length <= triggerSelection.max) || (triggerSelectionUsesChoice && triggerChoice && (triggerChoice !== "discard" || triggerSelectionKeys.length === 1))));
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {triggerResponse && responseDecisionReady && mandatoryChoiceTriggerOption?.selection?.type === "choice" && <MandatoryChoiceDialog option={mandatoryChoiceTriggerOption} selection={mandatoryChoiceTriggerOption.selection} selectedChoice={triggerChoice} selectedKeys={triggerSelectionKeys} disabled={responseControlsDisabled} busy={busy} error={error} onChoice={(choice) => { setTriggerChoice(choice); setTriggerSelectedKeys([]); }} onToggle={(key) => setTriggerSelectedKeys([key])} onConfirm={(choice, cardKeys) => onAction("trigger", { providerId: mandatoryChoiceTriggerOption.effectId, choice, ...(choice === "discard" ? { cardKeys } : {}) })} />}
...
export function MandatoryChoiceDialog({ option, selection, selectedChoice, selectedKeys, disabled, busy, error, onChoice, onToggle, onConfirm }: { option: TriggerOptionView; selection: ChoiceTriggerSelection; selectedChoice: string; selectedKeys: string[]; disabled: boolean; busy: boolean; error: string; onChoice: (choice: string) => void; onToggle: (key: string) => void; onConfirm: (choice: string, cardKeys: string[]) => void }) {
  ...
  const needsHandCard = selectedChoice === "discard";
  const complete = Boolean(selectedChoice && (!needsHandCard || validSelectedKeys.length === 1));
