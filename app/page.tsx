...
function pendingTimelineSequence(room: Room) {
  ...
  const initiatingIndex = [...room.timeline].map((event, index) => ({ event, index })).reverse().find(({ event }) => event.type === "card" && event.action !== "gain" && event.action !== "reveal" && (sequenceStartCardId ? event.card.id === sequenceStartCardId : publicPlayerName(event.player) === sourceName && (!expectedName || cardDefinition(event.card.kind).name === expectedName)))?.index ?? -1;
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_green_dragon" | "pass_green_dragon" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_response_timer" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className={`play-table ${sequenceEvents.length > 0 ? "sequence-active" : ""} ${resolutionClosing ? "sequence-concluding" : ""}`}>
      ...
      {room.players.map((player, index) => { const relativeIndex = (index - myTableIndex + room.players.length) % room.players.length; const angle = 180 + (360 / room.players.length) * relativeIndex; const radians = angle * Math.PI / 180; const targetable = Boolean(card && (isAttackCard(card) ? (player.distance ?? 99) <= (me?.attackRange ?? 1) : card.kind === "Dismantle" ? player.handCount > 0 : card.kind === "Steal" ? player.handCount > 0 && (player.distance ?? 99) <= 1 : card.kind === "Duel" ? true : card.kind === "Overindulgence" ? !player.judgementCards.some((delayed) => delayed.kind === "Overindulgence") : card.kind === "RationsDepleted" ? (player.distance ?? 99) <= 1 && !player.judgementCards.some((delayed) => delayed.kind === "RationsDepleted") : false)); return <button disabled={!room.isMyTurn || !canPlay || !player.alive || player.id === room.meId || !targetable} onClick={() => { setTarget(player.id); setTargetCardIndex(null); }} className={`started-player play-seat ${player.id === room.meId ? "self-player" : ""} ${player.seat === room.turnSeat && room.status === "playing" ? "active-turn" : ""} ${player.id === room.actionPlayerId && room.status === "playing" ? "active-action" : ""} ${target === player.id ? "targeted" : ""} ${!player.alive ? "defeated" : ""}`} key={player.id} style={{ "--angle": `${angle}deg`, "--countdown-x": `${Math.cos(radians) * 76}px`, "--countdown-y": `${Math.sin(radians) * 54}px` } as React.CSSProperties}><div className="seal">{player.name[0]}</div><b>{player.name}</b><small>{heroName(player.hero)} · {player.role ?? "Role hidden"}</small><span>{"♥".repeat(player.hp ?? 0)} · {player.handCount} cards{player.id !== room.meId ? ` · distance ${player.distance}` : ` · Attack Range ${player.attackRange}`}</span>{player.equipmentCards.length > 0 && <em className="equipment-zone">Weapon · {player.equipmentCards.map((equipment) => cardDefinition(equipment.kind).name).join(", ")}</em>}{player.judgementCards.length > 0 && <em className="judgement-zone">Judgement · {player.judgementCards.map((delayed) => cardDefinition(delayed.kind).name).join(", ")}</em>}{seatCountdown?.playerId === player.id && <Countdown key={seatCountdown.key} durationMs={seatCountdown.durationMs} deadline={seatCountdown.deadline} label={seatCountdown.label} />}</button>; })}
