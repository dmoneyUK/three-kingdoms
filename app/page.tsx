...
export function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: GameplayAction, extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  const responseCardAllowed = (item: Card) => selectedResponseProvider?.selection?.type === "cards" ? selectedResponseProvider.selection.eligibleCardIds.includes(item.id) : !semanticResponseOptions.length && Boolean(requiredResponseKind) && (rockCleavingResponse || (requiredResponseKind === "Negation" ? item.kind === "Negation" : requiredResponseKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge"));
  const responsePlayAction = genericResponse && selectedResponseProvider ? "respond" as GameplayAction : (["respond_negation", "respond_green_dragon", "respond_group", "respond_duel", "respond_dodge"] as GameplayAction[]).find((action) => canUseAction(room.currentAction, action)) ?? null;
