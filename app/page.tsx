...
type CardEvent = { id: string; player: string; target: string; card: Card; action?: "play" | "discard" | "gain" };
type CardGroupEvent = { id: string; type: "cards"; player: string; target: string; cards: Card[]; action: "discard" | "reveal" };
...
type Room = { code: string; status: "lobby" | "heroes" | "started" | "playing" | "finished"; maxPlayers: number; isHost: boolean; meId: string; myRole: string | null; myHeroOptions: Hero[]; players: Player[]; myHand: Card[]; turnSeat: number | null; phase: string | null; deckCount: number; discardTop: Card | null; log: string[]; timeline: GameEvent[]; isMyTurn: boolean; actionPlayerId: string | null; actionReason: string; isMyAction: boolean; pendingAttack: { sourceId: string; targetId: string } | null; pendingDuel: { sourceId: string; targetId: string; actorId: string; opponentId: string } | null; pendingGroup: { cardKind: "BarbarianInvasion" | "RainingArrows"; sourceId: string; actorId: string; requiredKind: "Attack" | "Dodge" } | null; pendingHarvest: { sourceId: string; actorId: string; revealed: Card[] } | null; pendingDying: { sourceId: string; targetId: string; deadline: number } | null };
...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className="play-table">
      ...
      {room.pendingHarvest && !presentationBusy && <div className="game-event-stage harvest-choice-stage" role="dialog" aria-label="Bumper Harvest card choice"><div><span>BUMPER HARVEST</span><b>{canChooseHarvest ? "Choose one revealed card" : `Waiting for ${actor?.name ?? "the next player"} to choose`}</b><small>Players choose in normal turn order, starting with the player who used the card.</small><div className="harvest-card-row">{room.pendingHarvest.revealed.map((choice) => <button type="button" className="harvest-card-choice" disabled={!canChooseHarvest || busy} key={choice.id} onClick={() => onAction("choose_harvest", { cardId: choice.id })}><CardFace card={choice} /></button>)}</div></div></div>}
