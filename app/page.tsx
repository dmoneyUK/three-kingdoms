...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "rescue_self" | "accept_defeat", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  useEffect(() => { if (activeEvent || !eventQueue.length) return; setActiveEvent(eventQueue[0]); setEventQueue((queue) => queue.slice(1)); }, [activeEvent, eventQueue]);
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b><span>{current?.name ?? "—"}&apos;s turn · {phaseName(room.phase)}</span></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <footer className="play-command"><div className="turn-controls"><span>{presentationBusy ? "Resolving the current game event…" : room.status === "finished" ? "The match has ended" : room.phase === "dying" ? room.pendingDying?.targetId === room.meId ? "Dying · use Peach to return to 1 HP, or accept defeat" : `${dyingPlayer?.name ?? "A player"} is deciding whether to use Peach` : room.phase === "response" ? room.pendingAttack?.targetId === room.meId ? `${attacker?.name ?? "An opponent"} attacked you — Dodge prevents all damage` : `${defender?.name ?? "The defender"} is answering ${attacker?.name ?? "the attacker"}'s Strike` : room.phase === "discard" && room.isMyTurn ? `Discard Phase · select ${excessCards} card${excessCards === 1 ? "" : "s"} together (${discardSelected.length}/${excessCards})` : room.isMyTurn ? room.phase === "draw" ? "Draw Phase · drawing two cards automatically…" : card?.kind === "Strike" && !target ? "Play Phase · choose an adjacent opponent (distance 1)" : room.phase === "play-struck" ? "Play Phase · Strike used; play Peach or finish Play" : "Play Phase · use permitted cards in any order" : `Waiting for ${current?.name ?? "another player"}`}</span><div>{room.phase === "dying" && room.pendingDying?.targetId === room.meId && <><button className="primary" disabled={busy || presentationBusy || !room.myHand.some((item) => item.kind === "Peach")} onClick={() => onAction("rescue_self")}>Use Peach · Return to 1 HP</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("accept_defeat")}>No Peach · Accept defeat</button></>}{room.phase === "response" && room.pendingAttack?.targetId === room.meId && <><button className="primary" disabled={busy || presentationBusy || !room.myHand.some((item) => item.kind === "Dodge")} onClick={() => onAction("respond_dodge")}>Play Dodge · No damage</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("take_damage")}>No Dodge · Take damage</button></>}{room.isMyTurn && room.phase === "discard" && <button className="end" disabled={busy || presentationBusy || discardSelected.length !== excessCards} onClick={() => onAction("discard_cards", { cardIds: discardSelected })}>Discard {excessCards} selected</button>}{room.isMyTurn && canPlay && <><button className="primary" disabled={busy || presentationBusy || !card || (card.kind === "Strike" && (!target || room.phase === "play-struck")) || card.kind === "Dodge"} onClick={play}>Play selected</button><button className="end" disabled={busy || presentationBusy} onClick={() => onAction("end_turn")}>Finish Play Phase</button></>}</div></div>
      <div className="play-hand">{room.myHand.map((item) => { const isSelected = room.phase === "discard" ? discardSelected.includes(item.id) : selected === item.id; return <button key={item.id} disabled={!room.isMyTurn || presentationBusy || (!canPlay && room.phase !== "discard")} onClick={() => { if (room.phase === "discard") setDiscardSelected((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : ids.length < excessCards ? [...ids, item.id] : ids); else setSelected((id) => id === item.id ? "" : item.id); setTarget(""); }} className={`game-card ${item.kind.toLowerCase()} ${isSelected ? "selected" : ""}`}><span className="corner">{item.rank}<i>{item.suit}</i></span><span className="card-glyph">{item.kind === "Strike" ? "⚔" : item.kind === "Dodge" ? "盾" : "桃"}</span><strong>{item.kind}</strong><small>{room.phase === "discard" ? isSelected ? "Selected · tap to remove" : "Tap to select" : item.kind === "Dodge" ? "Play when attacked" : item.kind === "Peach" ? "Recover 1 HP" : "Range 1 · deal 1 damage"}</small></button>; })}</div>
