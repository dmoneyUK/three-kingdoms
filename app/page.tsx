...
function GameRoom({ room, busy, error, drawNotice, onDrawNoticeEnd, onAction, onLeave }: { room: Room; busy: boolean; error: string; drawNotice: number; onDrawNoticeEnd: () => void; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => void; onLeave: () => void }) {
  ...
  return <main className="game-shell"><header className="topbar"><Brand /><div className="room"><span className="live-dot" /> ROOM <b>{room.code}</b></div><div className="top-actions"><button type="button" className="text-button history-button" onClick={() => setHistoryOpen(true)}>Event history</button><button className="text-button" onClick={onLeave}>Exit</button></div></header>
    ...
    <section className="play-table">
      ...
      {canRescue && !presentationBusy && <div className="game-event-stage rescue-decision-stage" role="alert"><div><span>PRIVATE RESCUE DECISION</span><b>{dyingPlayer?.name ?? "A player"} is dying</b><small>You have 5 seconds to give one Peach, or choose Do not give.</small></div></div>}
