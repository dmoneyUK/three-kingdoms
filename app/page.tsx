...
function GameRoom({ room, busy, error, onAction, onLeave }: { room: Room; busy: boolean; error: string; onAction: (action: "draw" | "play_card" | "end_turn" | "respond_dodge" | "take_damage" | "respond_duel" | "take_duel_damage" | "respond_group" | "take_group_damage" | "respond_negation" | "pass_negation" | "preview_harvest" | "choose_harvest" | "discard_cards" | "start_rescue_timer" | "give_peach" | "skip_rescue", extra?: Record<string, unknown>) => Promise<boolean>; onLeave: () => void }) {
  ...
  useEffect(() => { if (!canRespond) { automaticDamage.current = ""; return; } const requiredKind = room.pendingNegation ? "Negation" : room.pendingDuel || room.pendingGroup?.requiredKind === "Attack" ? "Attack" : "Dodge"; if (busy || room.myHand.some((item) => requiredKind === "Negation" ? item.kind === "Negation" : requiredKind === "Attack" ? isAttackCard(item) : item.kind === "Dodge")) return; const key = room.pendingNegation ? `negation-${room.pendingNegation.actorId}-${room.pendingNegation.negated}` : room.pendingGroup ? `group-${room.pendingGroup.cardKind}-${room.pendingGroup.actorId}` : room.pendingDuel ? `duel-${room.pendingDuel.actorId}-${room.pendingDuel.opponentId}` : `attack-${room.pendingAttack?.sourceId}-${room.pendingAttack?.targetId}`; if (automaticDamage.current === key) return; automaticDamage.current = key; onActionRef.current(room.pendingNegation ? "pass_negation" : room.pendingGroup ? "take_group_damage" : room.pendingDuel ? "take_duel_damage" : "take_damage"); }, [canRespond, busy, room.myHand, room.pendingAttack, room.pendingDuel, room.pendingGroup, room.pendingNegation]);
