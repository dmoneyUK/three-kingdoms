...
Triggered providers also declare a small semantic outcome instead of asking the route to recognize their identity: follow-up Attack, forced damage, damage prevention, or a non-terminal reaction that keeps the current event open. A non-terminal provider is excluded after it resolves and the remaining options are rediscovered from live state. This is the foundation for several optional equipment/hero reactions to one domain event without losing later choices.
