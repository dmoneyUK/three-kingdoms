...
- Normal card-response decisions now allow 10 seconds instead of 5. The server starts a fresh, actor-specific window every time a response card creates the next decision: for example, Dodge gets 10 seconds after Attack, then Rock Cleaving Axe gets a new 10 seconds after Dodge. Only the active responder sees the countdown; every one still supports an immediate Skip action. Peach rescue remains a separate 5-second decision.
...
- Frost Sword is playable with Attack Range 2. When its Attack would deal damage — including against an undefended bot — its owner gets the same centred, fresh 10-second choice pattern as Rock Cleaving Axe: **Discard up to 2 cards** from the target's Hand, Equipment Zone or Judgement Zone, or **Deal 1 damage**. The discard option remains legal if only one target card is available, and its deadline safely resolves to damage if the owner does nothing.
