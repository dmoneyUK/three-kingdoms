...
- a focused quick-test opening hand containing every non-weapon Standard card plus Frost Sword; other weapons remain in the deck, while Player 3 begins with three Attacks.
...
- On narrow portrait screens, both Frost Sword result buttons remain visible in the modal; the responsive footer-only button hiding rule no longer hides the discard choice.
...
- Frost Sword is playable with Attack Range 2. When its Attack would deal damage — including against an undefended bot — its owner gets the same centred, fresh 30-second human choice pattern as Rock Cleaving Axe (10 seconds for a bot): **Discard up to 2 cards** from the target's Hand, Equipment Zone or Judgement Zone, or **Deal 1 damage**. The discard option remains legal if only one target card is available, and its deadline safely resolves to damage if the owner does nothing.
