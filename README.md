...
The game surface keeps Exit available independently of the hidden top bar. Event History is the only event window and can be folded away to free table space; the separate turn-status strip is intentionally omitted from the board.
...
The current semantic layer discovers Negation through any legal provider and uses the shared event requirement for counter-windows, shares event-specific trigger continuation for human seats, and evaluates secondary Judgement once before dispatching a shared semantic outcome across Attack, Group, Duel, and Negation continuations. Canonical damage reactions now enter through generic `damage_about_to_apply` provider discovery rather than a Frost Sword gate. Canonical `respond` and `decline_response` requests retain that semantic dispatch boundary, the current client submits only canonical response/trigger actions, and newly-created decisions retain their transition-owned presentation event IDs. The client no longer contains dead Rock Cleaving/Frost/Green Dragon control branches; all active trigger choices use the generic provider renderer. The local validation suite includes semantic trigger reopen/exhaustion coverage for both `attack_dodged` and `damage_about_to_apply`; the latter proves two synthetic reactions reopen in order, apply original damage exactly once, and route lethal damage into the shared Dying/Peach flow. It also races duplicate response, trigger, lethal-damage, and Peach submissions and submits an old action revision, proving compare-and-swap claims allow one transition and leave no room in `resolving`. The tested human-seat guarantees are authoritative actor ownership, correct perspective switching, private hand/provider projection, wrong-seat rejection, stable resolution identity, and stale/double-submission safety. Synthetic providers are isolated to explicit test-Worker registration and absent from production registries. Saved-room compatibility remains isolated at the boundary. Semantic response/trigger architecture is complete; Blue Steel Sword implementation is now in progress.
