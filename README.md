...
Current Stage 6 milestone: Hero skill controls — Wusheng UI is projection-driven for Guan Yu in Play Phase and response windows, and Zhao Yun's Longdan now provides the same dedicated, projection-driven control for Dodge-as-Attack and Attack-as-Dodge. Longdan uses only hand cards, preserves native-card defaults, submits the existing `play_card`/`respond` protocol, and carries `playedAs: "attack" | "dodge"` as presentation metadata. Final Wusheng and Borrowed Sword cleanup is complete: Wusheng virtual-Attack cards enter that mode only through the dedicated hero-skill control, and Borrowed Sword target selection is isolated from generic response controls, hidden behind presentation barriers, and rejected server-side when no forced-Attack target exists. Borrowed Sword forced Attacks now retain `origin: "borrowed_sword"` and the original turn owner separately from the Attack source, allowing the holder's Frost Sword/Kirin Bow and weapon-trigger continuations to resolve while control returns to the Borrowed Sword user, including after Dying. Shared damage triggers now wait only for the current Attack's essential card presentation, never its informational “would damage” message; Kirin Bow and Frost Sword therefore open without the damage-trigger deadlock. Yin-Yang Swords target choices are now mandatory through generic trigger metadata: the target receives only discard or attacker-draw, with no Skip reaction when the trigger is non-declineable. Mandatory choice triggers now open directly in a centred table dialog; Yin-Yang's draw wording and concealed hand-card discard flow use the same readable picker treatment as other table decisions, while the server `trigger` capability and payload remain unchanged. Mandatory Yin-Yang choices now also project `timeoutChoiceId: "draw"`, so expiry submits the existing `trigger` action for attacker draw instead of leaving the match stuck or discarding randomly. Quick Test now uses a proper shuffled Standard deck while seeding Player1/Guan Yu with Frost Sword, Red Hare, Peach and Attack, Player2 with Kirin Bow and Nio Shield, and Player3/Zhao Yun with Blue Steel Sword, Dodge and Attack; Yin-Yang Swords and Borrowed Sword are also dealt to three distinct random open seats, and all remaining slots are filled from the shuffled deck. Generic target-card reactions now use a centred, projection-driven picker with readable equipment faces, concealed eligible hand positions, selection counts, stale-key filtering, and in-panel actions; hidden hand cards now come directly from authoritative `eligibleKeys`, so a presentation-only `handCount` cannot hide a valid Frost Sword choice. Passive equipment prevention now uses the same informational Game Messages and brief “Effect Triggered” notice path: Nio Shield visibly reports when it blocks a black Attack. Next milestone: the next individually verified Standard hero.
...
- a single-device Quick Test controller that follows the legal acting seat and shows only that player's hand in the normal bottom area. Perspective changes never trigger a private draw. Quick Test names its human-style seats Player1 through Player4, assigns Guan Yu to Player1 and Zhao Yun to Player3, seeds the first three seats with the requested Standard equipment plus a Dodge and Attack for Longdan (Frost Sword plus Red Hare; Kirin Bow plus Nio Shield; Blue Steel Sword plus Dodge and Attack), deals Yin-Yang Swords and Borrowed Sword to three distinct random seats with opening-hand capacity, preserves red Wusheng-capable cards for Player1, and fills all remaining opening slots from the shuffled deck.
