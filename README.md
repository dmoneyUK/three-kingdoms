...
The remaining legacy response and trigger names are now isolated in `game/compat/legacy-actions.ts`. They are accepted only to keep saved clients compatible, then translated into the canonical `respond`, `decline_response`, `trigger`, or `decline_trigger` flow before gameplay resolution begins. New UI and capability work must not add names to that adapter.
