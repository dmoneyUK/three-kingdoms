...
The current semantic layer discovers Negation through any legal provider and uses the shared event requirement for counter-windows, shares event-specific trigger continuation for human and bot decisions, and evaluates secondary Judgement once before dispatching a shared semantic outcome across Attack, Group, Duel, and Negation continuations. Canonical damage reactions now enter through generic `damage_about_to_apply` provider discovery rather than a Frost Sword gate. Canonical `respond` and `decline_response` requests retain that semantic dispatch boundary, the current client submits only canonical response/trigger actions, and newly-created decisions retain their transition-owned presentation event IDs. The client no longer contains dead Rock Cleaving/Frost/Green Dragon control branches; all active trigger choices use the generic provider renderer. The local validation suite includes semantic trigger reopen/exhaustion coverage for both `attack_dodged` and `damage_about_to_apply`; the latter proves two synthetic reactions reopen in order, apply original damage exactly once, and route lethal damage into the shared Dying/Peach flow. It now also races duplicate response, trigger, lethal-damage, and Peach submissions and submits an old action revision, proving compare-and-swap claims allow one transition and leave no room in `resolving`. Saved-room compatibility remains isolated at the boundary. No new cards should be added before the final architecture audit is signed off.
...
The current focus is **architecture stabilisation before the next weapon**. Normal Attack cards, Serpent Spear-formed Attacks and Green Dragon Blade follow-ups enter one shared semantic Attack declaration path, preserving the physical source card when downstream rules need its provenance. `game/pending.ts` now persists a canonical requirement-centric `ResponsePending` for Attack, Duel, AOE and Negation decisions; it stores what the actor must satisfy plus a small domain continuation, rather than making React reconstruct that meaning from one of several legacy pending shapes. The API publishes `currentAction` v3 with the private semantic requirement, eligible providers, `decline_response`, and `activation: "implicit" | "explicit"`. Ordinary physical cards are implicit—select a Dodge/Attack/Negation card directly—while abilities such as Eight Trigrams and Qingguo remain deliberate explicit choices. Legacy projected pending fields and provider-specific actions remain only as compatibility adapters while the protocol migration completes. Response capability discovery now has semantic `ActionRequirement`, `ResponseProvider`, and selection-constraint types; physical cards, Eight Trigrams, Serpent Spear and Qingguo are registered providers. Delayed Stratagems animate from the centre into the target's Judgement Zone, and Quick Test seeds Player 1 with Serpent Spear. Quick Test gameplay actions validate a live controller revision, return the newest room on stale actions, and serialize timeout/manual submissions. Normal multiplayer now has explicit human-seat switching regressions: the authenticated actor receives the private legal providers/options, the prior and other seats see no private choices, the resolution identity remains stable, stale or duplicate submissions are rejected, and both ordinary Dodge responses and Green Dragon trigger chains resume once. The UI no longer invents Dodge or damage controls for an unknown response state.
...
The regression runner now applies the tracked D1 migrations into an isolated `.wrangler/test-state` database before starting its Worker. This makes fresh CI checkout tests reproducible without modifying the developer's local game database. Green Dragon Blade and Rock Cleaving Axe execute through provider-owned `attack_dodged` triggers, while Frost Sword is the representative `damage_about_to_apply` trigger: each equipment module owns availability and live selection validation rather than route code inspecting a weapon. Response decisions now carry a `readyAfterEventId`; the client waits only for that decision's required public event, not for an unrelated presentation queue to drain. Canonical damage triggers, human and bot Attack-dodged triggers, and initial/counter Negation windows capture the exact event ID returned by event creation. Every Attack origin now enters the same damage-about-to-apply discovery path, including physical, Serpent Spear, and triggered follow-up Attacks. The Worker bundle includes inert synthetic semantic fixtures for the isolated E2E harness: the real API/D1 tests exercise unknown Attack, zero-card Dodge, and zero-card Negate providers, while the production registry has no active test capability unless the test-only hero or equipment markers are persisted.
...
The shared turn and response engine now uses effective horse-adjusted distance consistently in both UI and API, auto-resolves impossible Dodge responses, and keeps Quick Test at three HP with named mounts in the deck. Equipment and delayed Stratagem presentations now settle directly into their owner's Equipment or Judgement Zone without a duplicate numbered copy. Attack cards, Serpent Spear formations, Green Dragon follow-ups, and bot Attacks converge on a shared imminent-damage transition. Quick Test controller races are guarded by an authoritative action revision, and invalid generic response fallbacks are suppressed. Room reads are non-mutating and D1-efficient. Canonical response and trigger decisions persist precise fresh presentation barriers; secondary Judgement resumes through one semantic continuation boundary across Attack, Group, Duel, and Negation. The Worker/D1 suite now covers both an Attack-dodged event and a `damage_about_to_apply` event reopening two inert synthetic `continue_event` providers in order; the damage proof confirms the original Attack damage resumes exactly once after exhaustion. Old saved pending shapes remain isolated in adapters. The remaining 108-card manifest audit is tracked in `ROADMAP.md`.

## Roadmap

Negation now resolves each target separately: its initial window starts at the target and includes the Stratagem user. Passing is final within that opportunity; playing Negation opens a new counter window after its player. Once everyone passes, parity determines whether the normal Dodge/Attack/Serpent Spear response opens, with a fresh timer. Eight Trigrams can now provide an optional Judgement-based Dodge for both physical and Serpent Spear-formed Attacks. Successful Negation now has one pure semantic transition for human, bot, and Judgement paths, with focused parity/depth regression coverage; a successful Judgement with no remaining responder persists that transitioned state before resolution. The real Worker/D1 E2E proof for unknown semantic Attack, zero-card Dodge, and zero-card Negate providers is green, and the canonical response boundary preserves 0, 1, and many provider-reported card costs. The broader architecture milestone remains incomplete until the full exit checklist is independently closed. Do not begin Blue Steel Sword yet.
