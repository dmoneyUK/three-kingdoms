...
The latest architecture pass routes physical, Serpent Spear, and triggered follow-up Attacks through the same damage-about-to-apply provider discovery. Group/AOE target advancement now assigns a fresh presentation event ID to each next response barrier. Saved-client Frost compatibility is only projected when Frost is an actually available provider; generic damage events are not mislabeled as Frost Sword.
