...
- a single-device Quick Test controller that follows the legal acting seat and shows only that player's hand in the normal bottom area. Perspective changes never trigger a private draw. Quick Test names its human-style seats Player1 through Player4, assigns Guan Yu to Player1, seeds the first three seats with the requested Standard equipment (Frost Sword plus Red Hare; Kirin Bow plus Nio Shield; Blue Steel Sword), deals Yin-Yang Swords and Borrowed Sword to two distinct random seats with opening-hand capacity, preserves red Wusheng-capable cards for Player1, and fills all remaining opening slots from the shuffled deck.
