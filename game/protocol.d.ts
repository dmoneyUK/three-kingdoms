...
export type TriggerOptionView = { effectId: string; label: string; description?: string; allowDecline?: boolean; timeoutChoiceId?: string; selection: { type: "cards"; min: number; max: number; eligibleCardIds: string[]; targetIds?: string[] } | { type: "target"; targetIds: string[]; min?: number; max?: number } | { type: "target_cards"; targetId: string; min: number; max: number; eligibleKeys: string[] } | { type: "choice"; choices: { id: string; label: string }[]; eligibleHandKeys: string[]; cardCountByChoice?: Record<string, number> } | null };
export type PlayPhaseActionView = { cardId: string; canPlayAs: "attack" };
export type CurrentAction = { version: 3; kind: PendingKind | "response" | "turn" | "none"; actorId: string | null; deadline: number; reason: string; legalActions: GameplayAction[]; canDeclareAttack?: boolean; playPhaseActions?: PlayPhaseActionView[]; requirement?: ResponseRequirement; options?: ResponseOptionView[]; triggerEvent?: "turn_start" | "draw_phase" | "discard_phase" | "judgement_revealed" | "judgement_effective" | "attack_targeted" | "attack_dodged" | "damage_about_to_apply" | "damage_suffered" | "hero_choice" | "hand_lost"; triggerOptions?: TriggerOptionView[]; declineAction?: GameplayAction; distribution?: { cards: CardView[]; eligibleRecipientIds: string[] }; presentation?: { resolutionId: string | null; readyAfterEventId: string | null } };
export type CardView = { id: string; kind: string; suit: string; rank: string };
