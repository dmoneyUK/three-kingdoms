...
export type ResponseOptionView = { providerId: string; satisfies: ResponseRequirement; activation: "implicit" | "explicit"; label: string; selection: ResponseSelection; playedAs?: "attack" };
export type TriggerOptionView = { effectId: string; label: string; allowDecline?: boolean; timeoutChoiceId?: string; selection: { type: "cards"; min: number; max: number; eligibleCardIds: string[] } | { type: "target_cards"; targetId: string; min: number; max: number; eligibleKeys: string[] } | { type: "choice"; choices: { id: string; label: string }[]; eligibleHandKeys: string[] } | null };
export type PlayPhaseActionView = { cardId: string; canPlayAs: "attack" };
export type CurrentAction = { version: 3; kind: PendingKind | "response" | "turn" | "none"; actorId: string | null; deadline: number; reason: string; legalActions: GameplayAction[]; canDeclareAttack?: boolean; playPhaseActions?: PlayPhaseActionView[]; requirement?: ResponseRequirement; options?: ResponseOptionView[]; triggerEvent?: "attack_targeted" | "attack_dodged" | "damage_about_to_apply"; triggerOptions?: TriggerOptionView[]; declineAction?: GameplayAction; presentation?: { resolutionId: string | null; readyAfterEventId: string | null } };
