...
export type TriggerOptionView = { effectId: string; label: string; selection: { type: "cards"; min: number; max: number; eligibleCardIds: string[] } | { type: "target_cards"; targetId: string; min: number; max: number; eligibleKeys: string[] } | null };
export type CurrentAction = { version: 3; kind: PendingKind | "response" | "turn" | "none"; actorId: string | null; deadline: number; reason: string; legalActions: GameplayAction[]; requirement?: ResponseRequirement; options?: ResponseOptionView[]; triggerEvent?: "attack_dodged" | "damage_about_to_apply"; triggerOptions?: TriggerOptionView[]; declineAction?: GameplayAction; presentation?: { resolutionId: string | null; readyAfterEventId: string | null } };
