...
export const rockCleavingAxeDodgedAttackTrigger: TriggeredEffect = {
  ...
  event: "attack_dodged",
  getOption: (context) => {
    const cards = context.sourceCards ?? [];
  ...
  resolve: (context, selection) => {
    ...
    const cards = ids.map((id) => (context.sourceCards ?? []).find((card) => card.id === id));
    return cards.every((card): card is NonNullable<typeof card> => Boolean(card))
      ? { status: "resolved", effectId: "rock_cleaving_axe_attack_dodged", consumeCardIds: ids }
      : null;
  },
};
