...
export type PassiveAttackContext = { targetEquipment: Card[]; sourceEquipment?: Card[]; attack: Card | null | undefined };
...
export function resolvePassiveAttackModifiers(context: PassiveAttackContext) {
  if (context.sourceEquipment?.some((card) => card.kind === "BlueSteelSword")) return null;
